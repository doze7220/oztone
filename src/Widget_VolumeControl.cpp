...
void VolumeControlWidget::UpdateLayout(const WidgetContext &ctx,
  ...
  D2D1_SIZE_F renderTargetSize = (ctx.drumBuffer.count(0) > 0 && ctx.drumBuffer.at(0).artBitmap) ? D2D1_SIZE_F{1024, 512} : D2D1_SIZE_F{1024, 512}; // Need actual context size, but we can just use DPI scale logic
}
