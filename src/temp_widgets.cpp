...
void PlaylistWidget::CreateResources(ID2D1DeviceContext* context, IWICImagingFactory* wicFactory, IDWriteFactory* dwriteFactory, const ConfigManager* config) {
    ...
    context->CreateSolidColorBrush(ParseHexColor(config->GetPlaylistBgColor()), &m_playlistBgBrush);
    context->CreateSolidColorBrush(ParseHexColor(config->GetPlaylistHighlightColor()), &m_playlistHighlightBrush);
...
void PlaylistWidget::UpdateLayout(const WidgetContext& ctx, const ConfigManager* config) {
    ...
    if (m_dwriteFactory && m_trackCountTextFormat && m_lastTotalTracks != ctx.totalTracks) {
        ...
        m_trackCountTextLayout.Reset();
        std::wstring trackCountText = std::to_wstring(ctx.totalTracks) + L" TRACKS";
        m_dwriteFactory->CreateTextLayout(
            trackCountText.c_str(),
            static_cast<UINT32>(trackCountText.length()),
...
void PlaylistWidget::Draw(ID2D1DeviceContext* context, const WidgetContext& ctx, const ConfigManager* config) {
    ...
    if (m_playlistSlideX < layout.playlistWidth - 0.5f) {
        ...
        float itemHeight = static_cast<float>(config->GetPlaylistItemOffsetY());
        float startY = layout.playlistY + layout.playlistHeight / 2.0f - (ctx.currentTrackIndex * itemHeight) + m_playlistManualScrollY;

        for (size_t i = 0; i < ctx.totalTracks; ++i) {
            float y = startY + i * itemHeight;
            
            if (y > layout.playlistY + layout.playlistHeight + itemHeight) break;
            if (y < layout.playlistY - itemHeight) continue;

            PlaylistItemLayout itemLayout = LayoutCalculator::CalculatePlaylistItemLayout(logicWidth, logicHeight, config, m_playlistSlideX, y);

            if (i == ctx.currentTrackIndex && m_playlistHighlightBrush) {
                m_playlistHighlightBrush->SetOpacity(config->GetPlaylistHighlightOpacity());
                context->FillRectangle(&itemLayout.highlightRect, m_playlistHighlightBrush.Get());
            }

            if (ctx.shuffleList && i < ctx.shuffleList->size()) {
                const std::wstring& filePath = (*ctx.shuffleList)[i];
                ...
                if (m_dwriteFactory && m_playlistTitleTextFormat) {
                    Microsoft::WRL::ComPtr<IDWriteTextLayout> titleLayout;
                    if (SUCCEEDED(m_dwriteFactory->CreateTextLayout(
                        title.c_str(), static_cast<UINT32>(title.length()),
                        m_playlistTitleTextFormat.Get(),
                        itemLayout.titleMaxWidth, itemLayout.textMaxHeight,
                        &titleLayout))) {
                        context->DrawTextLayout(itemLayout.titleOrigin, titleLayout.Get(), m_textBrush.Get());
                    }
                }
                
                if (m_dwriteFactory && m_playlistArtistTextFormat) {
                    Microsoft::WRL::ComPtr<IDWriteTextLayout> artistLayout;
                    if (SUCCEEDED(m_dwriteFactory->CreateTextLayout(
                        artist.c_str(), static_cast<UINT32>(artist.length()),
                        m_playlistArtistTextFormat.Get(),
                        itemLayout.artistMaxWidth, itemLayout.textMaxHeight,
                        &artistLayout))) {
                        context->DrawTextLayout(itemLayout.artistOrigin, artistLayout.Get(), m_playlistArtistBrush.Get());
                    }
                }

                if (m_dwriteFactory && m_playlistTimeTextFormat) {
                    std::wstring duration = L"0:00";
                    Microsoft::WRL::ComPtr<IDWriteTextLayout> timeLayout;
                    if (SUCCEEDED(m_dwriteFactory->CreateTextLayout(
                        duration.c_str(), static_cast<UINT32>(duration.length()),
                        m_playlistTimeTextFormat.Get(),
                        itemLayout.timeMaxWidth, itemLayout.textMaxHeight,
                        &timeLayout))) {
                        context->DrawTextLayout(itemLayout.timeOrigin, timeLayout.Get(), m_playlistTimeBrush.Get());
                    }
                }
            }
        }
...
void ResizeGripWidget::CreateResources(ID2D1DeviceContext* context, IWICImagingFactory* wicFactory, IDWriteFactory* dwriteFactory, const ConfigManager* config) {
    ...
    context->CreateSolidColorBrush(ParseHexColor(config->GetResizeGripColor()), &m_resizeGripBrush);
    ...
    if (d2dFactory) {
        ...
        if (m_resizeGripGeometry) {
            ...
            if (SUCCEEDED(m_resizeGripGeometry->Open(&sink))) {
                float size = config->GetResizeGripSize();
                sink->BeginFigure(D2D1::Point2F(-size, 0.0f), D2D1_FIGURE_BEGIN_FILLED);
                sink->AddLine(D2D1::Point2F(0.0f, -size));
