...
void Application::OnPlaylistClicked(int x, int y) {
  ...
  if (logicalY < toolbarHeight) {
    // 繝・・繝ｫ繝舌・鬆伜沺縺ｸ縺ｮ繧ｯ繝ｪ繝・け縺ｯWindow.cpp(WM_LBUTTONDOWN)縺九ｉ
    // SetPlaylistToolbarClickCallback繧帝壹§縺ｦ鬟帙ｓ縺ｧ縺上ｋ縺溘ａ縺薙％縺ｧ縺ｯ辟｡隕悶☆繧九・    return;
  }
...
void Application::SwitchPlaylist(const std::wstring &filepath) {
  ...
  // 蛻・ｊ譖ｿ縺亥燕縺ｫ迴ｾ蝨ｨ縺ｮ繝励Ξ繧､繝ｪ繧ｹ繝育憾諷九ｒ菫晏ｭ・  if (!m_playlistManager.IsEmpty()) {
  ...
  // 譌｢蟄倥・蜀咲函繧・く繝･繝ｼ繧偵け繝ｪ繧｢縺吶ｋ・・learPlaylist()
  // 縺ｯ繝輔ぃ繧､繝ｫ繧堤ｩｺ縺ｫ縺励※縺励∪縺・・縺ｧ蜻ｼ縺ｰ縺ｪ縺・ｼ・  m_audioPlayer.Stop();
  m_trackAnalyzer.ClearQueue();
  m_isPrefetchReady.store(false);
  m_renderer.SetTrackInfo(L"NO TRACK", L"---", 0, true);
  m_renderer.SetAlbumArt(nullptr);
...
void Application::CreateNewPlaylist() {
  ...
  // 譁ｰ縺励＞繝代せ縺ｫ螟画峩縺吶ｋ蜑阪↓迴ｾ蝨ｨ縺ｮ迥ｶ諷九ｒ菫晏ｭ・  if (!m_playlistManager.IsEmpty()) {
    m_playlistManager.SaveToFile(defaultPath);
  }
