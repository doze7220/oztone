...
-------------------------------------------------------------------------------
### 作業指示書 REQ: Phase 23-7 Task 2 : バックグラウンド表示モードの復活 (Hotfix)
*  ルール: D:\ozlab\oztone\PROJECT_CONSTITUTION.md
*  開発資料:D:\ozlab\oztone\PROJECT_ARCHITECTURE.md
*  実装計画書:D:\ozlab\oztone\_docs\logs\20260720_2100_RES_Phase23-7_PipelineCompletion.md

#### 【作業手順（厳守事項）】
本プロンプトはPhase 23-7 Task 2: バックグラウンド表示モードの復活 (Hotfix) である。必ず以下の順序で作業を行うこと。
1. ルール（PROJECT_CONSTITUTION.md）および開発資料（PROJECT_ARCHITECTURE.md）を熟読・把握すること。
2. 実装計画書（20260720_2100_RES_Phase23-7_PipelineCompletion.md）を読み、今回の自分のスコープが「タスク2」のみであることを確認すること。
3. 上記を確認した後、以下の【実装要件】に従って **「タスク2のみ」** の実装を開始し、ソースコードの修正を実行すること。絶対にタスク3以降をフライングで実行しないこと。
4. 作業完了後、既存の作業レポート（20260720_2100_RES_Phase23-7_PipelineCompletion.md）の「タスク2」のチェックボックスを完了 [x] にし、必ず以下のフォーマットで詳細作業内容を追記すること。
...
#### 【実装要件】
*   `src/Renderer_Draw.cpp` の `DrawBackground` 内における描画処理に、`m_config->GetBackgroundArtMode()` を評価する分岐を追加する。
*   モードが `1` (非表示) の場合は、背景画像の描画（クロスフェード処理等含む）を完全にスキップする。
*   モードが `2` (デフォルト固定) の場合は、現在や過去のWIC画像を無視し、`m_placeholderArtBitmap` を用いてデフォルトの背景を固定描画するようにする。
*   モードが `0` (再生中) の場合は、タスク1までに復元されたWIC画像に基づくクロスフェード描画を通常通り行う。
*   フェード進行度や過去画像のライフサイクル管理は `BackgroundManager` 側に委ね、`Renderer` は状態を持たずに「どう描画するか」の分岐のみを行うこと。
*   ダークオーバーレイ（`m_bgDarkenBrush` を用いた半透明黒矩形）の描画は、背景画像の描画モードに関わらず、背景画像処理の後に従来通り実行すること。

#### 【絶対遵守ルール (Constraints)】
*   **スコープの厳守** : 本タスクは背景の表示モード分岐の復活のみを行う。工事残骸コメントのパージ（タスク3）は絶対にフライングで行わないこと。

-------------------------------------------------------------------------------

### 作業指示書 REQ: Phase 23-7 Task 3 : 工事残骸マーカーのパージ
*  ルール: D:\ozlab\oztone\PROJECT_CONSTITUTION.md
*  開発資料:D:\ozlab\oztone\PROJECT_ARCHITECTURE.md
*  実装計画書:D:\ozlab\oztone\_docs\logs\20260720_2100_RES_Phase23-7_PipelineCompletion.md

#### 【作業手順（厳守事項）】
本プロンプトはPhase 23-7 Task 3: 工事残骸マーカーのパージである。必ず以下の順序で作業を行うこと。
1. ルール（PROJECT_CONSTITUTION.md）および開発資料（PROJECT_ARCHITECTURE.md）を熟読・把握すること。
2. 実装計画書（20260720_2100_RES_Phase23-7_PipelineCompletion.md）を読み、今回の自分のスコープが「タスク3」のみであることを確認すること。
3. 上記を確認した後、以下の【実装要件】に従って **「タスク3のみ」** の実装を開始し、ソースコードの修正を実行すること。
4. 作業完了後、既存の作業レポート（20260720_2100_RES_Phase23-7_PipelineCompletion.md）の「タスク3」のチェックボックスを完了 [x] にし、必ず以下のフォーマットで詳細作業内容を追記すること。
    ### タスク3: 工事残骸マーカーのパージ
    **【対象ファイル】**
    - `ファイル名` (編集)
    **【作業内容】**
    - 詳細な作業内容
5. チャットにて「タスク3の実装が完了しました。Phase 23の全タスクが完了しましたので、最終確認をお願いします」と報告すること。

#### 【実装要件】
*   `src/Renderer.h` および `src/Renderer.cpp` を検索し、「Phase23-1 背景アートパージに伴い削除」や「BackgroundManager結線待ちマーカー」といった、大解体・工事のために残されていた不要なコメントやマーカーをすべて検索し、行ごと削除してコードを徹底的にクリーンアップすること。

#### 【絶対遵守ルール (Constraints)】
*   **スコープの厳守** : 本タスクは不要なコメント・マーカーの削除のみを行う。既存の機能ロジックや動作に関するコード修正は絶対に行わないこと。
