...
-----------------------------------------------------------------------------------
### 作業指示書 REQ: Phase 20-9 Task 8 & 9 : Widgetsファイルの物理削除とドキュメント更新
以下のプロジェクトルールと開発資料を熟読すること。
*  D:\ozlab\oztone\PROJECT_CONSTITUTION.md
*  D:\ozlab\oztone\PROJECT_ARCHITECTURE.md
*  D:\ozlab\oztone\_docs\logs\20260712_2333_RES_Phase20-9_Widgets_Destruction.md
...
#### 【作業手順（厳守事項）】
1. 本プロンプトは Widgets.h / .cpp の物理削除およびドキュメント更新の実行 である。直ちに以下の【実装要件】に従ってファイル削除とドキュメントの修正を実行すること。
2. 作業完了後、既存の作業レポート（20260712_2333_RES_Phase20-9_Widgets_Destruction.md）の「タスク8」および「タスク9」のチェックボックスを完了 `[x]` にし、詳細作業内容を追記すること。
3. チャットにて「Phase 20-9 の全タスクが完了しました。Widgets.cppは完全に消滅しました！」と報告すること。

#### 【実装要件】
*   **要件1: 旧ファイルの物理削除とビルド設定の更新 (タスク8)**
    *   中身が完全に空になった `src/Widgets.h` および `src/Widgets.cpp` をプロジェクトから物理削除する。
    *   `CMakeLists.txt` のソースおよびヘッダリストから `src/Widgets.cpp` と `src/Widgets.h` のエントリを削除し、正常にビルドが通る状態にする。
*   **要件2: ドキュメントの更新 (タスク9)**
    *   `PROJECT_ARCHITECTURE.md` を確認し、「各具象 Widget 実装」等の項目に記載されている `Widgets.h/cpp` というまとめた記述を、今回分割した個別のファイル群（`Widget_AppLogo.h/cpp`, `Widget_TrackInfo.h/cpp` 等）のプレフィックス命名規則を反映した形へと更新する。

#### 【絶対遵守ルール (Constraints)】
*   **コード変更の禁止**: 本作業は不要ファイルの削除とドキュメント更新のみである。残存しているC++のロジック等には一切触れないこと。
