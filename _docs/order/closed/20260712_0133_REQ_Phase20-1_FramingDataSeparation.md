...
##### 作業指示書 REQ: Phase 20-1 Hotfix 1: ArtFramingDatabaseの保存タイミング修正 (実装実行)
以下のプロジェクトルールと開発資料を熟読すること。
*  D:\ozlab\oztone\PROJECT_CONSTITUTION.md
*  D:\ozlab\oztone\PROJECT_ARCHITECTURE.md
...
2. コード修正が完全に終わった後、既存の作業レポート（D:\ozlab\oztone\_docs\logs\20260712_0134_RES_Phase20-1_FramingDataSeparation.md）の末尾に「HOTFIX 1: ArtFramingDatabaseの保存タイミング修正」の項目を追加し、原因と対応内容を追記すること。
3. チャットにて「Hotfixの実装が完了しました。ビルド・動作確認をお願いします」と報告すること。

---

###### 【実装要件】
現在、`ArtFramingDatabase` の保存（`SaveToFile`）がアプリ終了時以外に発火していないバグを修正する。

*   **1. `Application.cpp`: 曲切り替え時のDB保存処理の追加**
    *   曲が切り替わるタイミングである以下の箇所において、既存のプレイリスト保存処理（`m_playlistManager.SaveToFile(...)`）の近く等に `m_framingDb.SaveToFile(m_framingDbPath);` を呼び出してフレーミング設定を確実に永続化する処理を追加する。
        *   `Application::Run` 内のオーディオ自動進行時（`m_audioPlayer.IsAtEnd()` による次曲移行前）
        *   `Application::HandleMediaCommand` 内の NEXT / PREV による曲スキップ時
        *   プレイリストのクリックコールバック内で実行されるジャンプ再生時（`m_playlistManager.JumpToIndex` 実行付近）
        *   `Application::SwitchPlaylist` や `Application::ClearPlaylist` の実行時（リストが破棄・切り替わる直前）
        
*   **2. 不要なコールバック処理の削除**
    *   `Application::Initialize` 内などに実装された `m_window.SetArtFramingSaveCallback` における `m_framingDb.SaveToFile` の呼び出しは、過去の最適化によって無効なルートとなっているため削除する。

--------------------------------------------------------------------------------

##### 作業指示書 REQ: Phase 20-1 Hotfix 2: ArtFramingDatabaseの未初期化メモリバグ修正 (実装実行)
以下のプロジェクトルールと開発資料を熟読すること。
*  D:\ozlab\oztone\PROJECT_CONSTITUTION.md
*  D:\ozlab\oztone\PROJECT_ARCHITECTURE.md
...
###### 【作業手順（厳守事項）】
1. 本プロンプトはHotfixの「実装実行」である。事前のレポート作成や計画立案は不要なので、直ちに以下の【実装要件】に従ってコードの修正を実行すること。
2. コード修正が完全に終わった後、既存の作業レポート（D:\ozlab\oztone\_docs\logs\20260712_0134_RES_Phase20-1_FramingDataSeparation.md）の末尾に「HOTFIX 2: ArtFramingDatabaseの未初期化メモリバグ修正」の項目を追加し、原因と対応内容を追記すること。
3. チャットにて「Hotfixの実装が完了しました。ビルド・動作確認をお願いします」と報告すること。

---

###### 【実装要件】
DBに未登録の曲に対してフレーミング操作を行った際、未初期化メモリのゴミデータ（不定値）が計算・保存されてアプリがクラッシュする重大なバグを修正する。

*   **1. `ArtFramingDatabase.cpp`: `GetFraming` メソッドの安全化**
    *   `GetFraming` メソッド内において、指定された `filepath` がマップ内に見つからなかった場合（`false` を返すルート）の直前で、引数の参照に対して安全なデフォルト値を代入するフォールバック処理を追加する。
    *   `outX = 0.0f; outY = 0.0f; outScale = 1.0f;` を代入してから `return false;` とすること。

*   **2. `Application.cpp`: 呼び出し側の変数初期化の徹底**
    *   `Application.cpp` 内で `m_framingDb.GetFraming` を呼び出しているすべての箇所（`Application::ForceRender`、右ドラッグ操作のコールバック、フレーミングリセットのコールバック、ホイール操作のコールバック等）を確認する。
    *   `GetFraming` の引数として渡すローカル変数の宣言を `float artX = 0.0f, artY = 0.0f, artScale = 1.0f;` のように、必ず明示的に初期化してから関数へ渡すよう徹底的に修正する。
