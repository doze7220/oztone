...
--------------------------------------------------------------------------------
### 作業指示書 REQ: Phase 20-5 Task 5 : LogoMenu設定の委譲
*  D:\ozlab\oztone\PROJECT_CONSTITUTION.md
*  D:\ozlab\oztone\PROJECT_ARCHITECTURE.md
*  D:\ozlab\oztone\_docs\logs\20260712_1644_RES_Phase20-5_ConfigManagerDelegation.md

#### 【作業手順（厳守事項）】
1. 本プロンプトはLogoMenu設定の委譲の「実装実行」である。直ちに以下の【実装要件】に従ってコードとドキュメントの修正を実行すること。
2. 作業完了後、既存の作業レポート（logs\20260712_1644_RES_Phase20-5_ConfigManagerDelegation.md）の「タスク5」のチェックボックスを完了 [x] にし、詳細作業内容を追記すること。
3. チャットにて「タスク5（LogoMenu設定の委譲）(Phase 20-5)がすべて完了しました。ビルド・動作確認をお願いします」と報告すること。

#### 【実装要件】
`ConfigManager.cpp` の `LoadSettings` や `ResetToDefaults` に残存しているアプリアイコン (AppLogo) およびロゴ拡張メニュー (LogoMenu) 関連の初期値設定およびINI読み込み処理を、オートフィルヘルパー関数を用いて `ConfigManager_LogoMenu.cpp` へ委譲し、コードを純化する。

*   **要件1: `LoadLogoMenuSettings()` の実装 (`src/ConfigManager_LogoMenu.cpp`)**
    *   `ConfigManager_LogoMenu.cpp` 内に `void ConfigManager::LoadLogoMenuSettings()` を実装する。
    *   `ConfigManager.cpp` の `LoadSettings()` 内で行われていた以下の設定項目の読み込み処理をこちらへ移行する。
        *   アプリアイコン (`[Layout_AppLogo]`)
        *   ロゴ拡張メニュー (`[Layout_LogoMenu]`) 関連の全項目
    *   オートフィルヘルパー関数（`LoadOrWriteInt`, `LoadOrWriteFloat`, `LoadOrWriteString` 等）を使用し、「メモリ変数の初期化」と「INIからの読み書き」を1行で完結させる構造へリファクタリングする。

*   **要件2: 大元からの処理削除と司令塔化 (`src/ConfigManager.cpp`)**
    *   大元の `ConfigManager.cpp` の `LoadSettings()` 内から、AppLogo および LogoMenu 関連の読み込みロジックを全て削除し、代わりに `LoadLogoMenuSettings();` を呼び出すように変更する。
    *   大元の `ConfigManager.cpp` の `ResetToDefaults()` 内から、関連する初期値代入ロジックを全て削除する。

#### 【絶対遵守ルール (Constraints)】
*   **スコープの厳守**: 今回は「LogoMenu（およびAppLogo）設定」に関する部分のみを委譲すること。VisualizerやSystem等の他の設定は今後のタスクで移行するため、大元の `LoadSettings` や `ResetToDefaults` にそのまま残しておくこと。
*   **動作の維持**: オートフィルヘルパー関数に渡すデフォルト値は、元々 `ResetToDefaults` や `LoadSettings` で使われていた値と完全に一致させること。

--------------------------------------------------------------------------------

### 作業指示書 REQ: Phase 20-5 Task 6 : Visualizer設定の委譲
*  D:\ozlab\oztone\PROJECT_CONSTITUTION.md
*  D:\ozlab\oztone\PROJECT_ARCHITECTURE.md
*  D:\ozlab\oztone\_docs\logs\20260712_1644_RES_Phase20-5_ConfigManagerDelegation.md

#### 【作業手順（厳守事項）】
1. 本プロンプトはVisualizer設定の委譲の「実装実行」である。直ちに以下の【実装要件】に従ってコードとドキュメントの修正を実行すること。
2. 作業完了後、既存の作業レポート（logs\20260712_1644_RES_Phase20-5_ConfigManagerDelegation.md）の「タスク6」のチェックボックスを完了 [x] にし、詳細作業内容を追記すること。
3. チャットにて「タスク6（Visualizer設定の委譲）(Phase 20-5)がすべて完了しました。ビルド・動作確認をお願いします」と報告すること。

#### 【実装要件】
`ConfigManager.cpp` の `LoadSettings` や `ResetToDefaults` に残存している Visualizer 関連の初期値設定およびINI読み込み処理を、オートフィルヘルパー関数を用いて `ConfigManager_Visualizer.cpp` へ委譲し、コードを純化する。

*   **要件1: `LoadVisualizerSettings()` の実装 (`src/ConfigManager_Visualizer.cpp`)**
    *   `ConfigManager_Visualizer.cpp` 内に `void ConfigManager::LoadVisualizerSettings()` を実装する。
    *   `ConfigManager.cpp` の `LoadSettings()` 内で行われていた以下の設定項目の読み込み処理をこちらへ移行する。
        *   基本設定 (`[Visualizer]`)：VisualizerMode, HighFreqNoiseThreshold, EnablePreScan, 5バンドEQゲイン群
        *   PrismBeat設定 (`[Visualizer_PrismBeat]`)：PrismLineThickness, PrismGlow1Thickness, PrismGlow1Opacity, PrismGlow2Thickness, PrismGlow2Opacity, MaxHeightRatio 等
        *   HaloDust設定 (`[Visualizer_HaloDust]`)：BaseRadiusRatio, GraphLengthRatio, HaloParticleSpeed, HaloLaserLengthRatio, HaloLaserSpeed, HaloParticleSize, HaloParticleSpawnRate, HaloLaserSpawnRate, HaloParticleLifeTime, HaloCoreThickness, HaloGlowThickness 等の無数の設定群
    *   オートフィルヘルパー関数（`LoadOrWriteInt`, `LoadOrWriteFloat`, `LoadOrWriteString` 等）を使用し、「メモリ変数の初期化」と「INIからの読み書き」を1行で完結させる構造へリファクタリングする。

*   **要件2: 大元からの処理削除と司令塔化 (`src/ConfigManager.cpp`)**
    *   大元の `ConfigManager.cpp` の `LoadSettings()` 内から、上記 Visualizer 関連の読み込みロジックを全て削除し、代わりに `LoadVisualizerSettings();` を呼び出すように変更する。
    *   大元の `ConfigManager.cpp` の `ResetToDefaults()` 内から、関連する初期値代入ロジックを全て削除する。

#### 【絶対遵守ルール (Constraints)】
*   **スコープの厳守**: 今回は「Visualizer設定」に関する部分のみを委譲すること。System設定（ホットキーやOSD等）は次タスクで移行するため、大元の `LoadSettings` や `ResetToDefaults` にそのまま残しておくこと。
*   **動作の維持**: オートフィルヘルパー関数に渡すデフォルト値は、元々 `ResetToDefaults` や `LoadSettings` で使われていた値と完全に一致させること。特に Visualizer 設定はパラメータが非常に多いため、漏れがないよう注意すること。

--------------------------------------------------------------------------------

### 作業指示書 REQ: Phase 20-5 Task 7 : System設定の委譲
*  D:\ozlab\oztone\PROJECT_CONSTITUTION.md
*  D:\ozlab\oztone\PROJECT_ARCHITECTURE.md
*  D:\ozlab\oztone\_docs\logs\20260712_1644_RES_Phase20-5_ConfigManagerDelegation.md

#### 【作業手順（厳守事項）】
1. 本プロンプトはSystem設定の委譲の「実装実行」である。直ちに以下の【実装要件】に従ってコードとドキュメントの修正を実行すること。
2. 作業完了後、既存の作業レポート（logs\20260712_1644_RES_Phase20-5_ConfigManagerDelegation.md）の「タスク7」のチェックボックスを完了 [x] にし、詳細作業内容を追記すること。
3. チャットにて「タスク7（System設定の委譲）(Phase 20-5)がすべて完了しました。ビルド・動作確認をお願いします」と報告すること。

#### 【実装要件】
`ConfigManager.cpp` の `LoadSettings` や `ResetToDefaults` に残存している System（グローバルホットキーおよびOSD）関連の初期値設定およびINI読み込み処理を、オートフィルヘルパー関数を用いて `ConfigManager_System.cpp` へ委譲し、コードを純化する。

*   **要件1: `LoadSystemSettings()` の実装 (`src/ConfigManager_System.cpp`)**
    *   `ConfigManager_System.cpp` 内に `void ConfigManager::LoadSystemSettings()` を実装する。
    *   `ConfigManager.cpp` の `LoadSettings()` 内で行われていた以下の設定項目の読み込み処理をこちらへ移行する。
        *   グローバルホットキー設定 (`[GlobalHotkeys]`): `ShowHotkeys`、チートシートのレイアウト設定、各アクションのキーバインド（修飾キー、仮想キー）
        *   OSD設定 (`[Layout_OSD]`): `EnableOSD`, `OsdFontFamily`, `OsdFontSize`, `OsdTextColor`, `OsdShadowColor`, `OsdShadowOpacity`, `OsdFadeSpeed`, `OsdFadeWait` 等
    *   オートフィルヘルパー関数（`LoadOrWriteInt`, `LoadOrWriteFloat`, `LoadOrWriteString` 等）を使用し、「メモリ変数の初期化」と「INIからの読み書き」を1行で完結させる構造へリファクタリングする。

*   **要件2: 大元からの処理削除と司令塔化 (`src/ConfigManager.cpp`)**
    *   大元の `ConfigManager.cpp` の `LoadSettings()` 内から、上記 System 関連の読み込みロジックを全て削除し、代わりに `LoadSystemSettings();` を呼び出すように変更する。
    *   大元の `ConfigManager.cpp` の `ResetToDefaults()` 内から、関連する初期値代入ロジックを全て削除する。

#### 【絶対遵守ルール (Constraints)】
*   **スコープの厳守**: 今回は「System設定（GlobalHotkeys, Layout_OSD）」に関する部分のみを委譲すること。
*   **動作の維持**: オートフィルヘルパー関数に渡すデフォルト値は、元々 `ResetToDefaults` や `LoadSettings` で使われていた値と完全に一致させること。

--------------------------------------------------------------------------------
### 作業指示書 REQ: Phase 20-5 Task 8 : ConfigManager.cpp の最終整理とドキュメント更新
*  D:\ozlab\oztone\PROJECT_CONSTITUTION.md
*  D:\ozlab\oztone\PROJECT_ARCHITECTURE.md
*  D:\ozlab\oztone\_docs\logs\20260712_1644_RES_Phase20-5_ConfigManagerDelegation.md

#### 【作業手順（厳守事項）】
1. 本プロンプトはPhase 20-5の最終整理およびドキュメント更新の「実装実行」である。直ちに以下の【実装要件】に従ってコードとドキュメントの修正を実行すること。
2. 作業完了後、既存の作業レポート（logs\20260712_1644_RES_Phase20-5_ConfigManagerDelegation.md）の「タスク8」のチェックボックスを完了 [x] にし、詳細作業内容を追記すること。
3. チャットにて「タスク8（最終整理とドキュメント更新）(Phase 20-5)がすべて完了し、ConfigManager究極の委譲化が完了しました。ビルド・動作確認をお願いします」と報告すること。

#### 【実装要件】
`ConfigManager.cpp` の最終的なクリーンアップを行い、アーキテクチャの変更をプロジェクト資料へ反映する。

*   **要件1: `ConfigManager.cpp` の最終整理**
    *   タスク1〜7の委譲作業によって不要となったインクルード文、未使用のローカル変数、空行、コメントの残骸などを完全に削除し、ファイルをクリーンアップする。
    *   `LoadSettings()` と `ResetToDefaults()` が、各サブメソッド（`LoadWindowSettings()`等）を順次呼び出すだけの美しい司令塔になっていることを最終確認・整形する。

*   **要件2: 開発資料の更新**
    *   `PROJECT_ARCHITECTURE.md` の `ConfigManager` クラスの解説セクションを更新し、「巨大な `LoadSettings` を解体し、各UIの物理分割先ファイル（`ConfigManager_Window.cpp`等）へ委譲してカプセル化したこと」「INIオートフィル機構（自己修復機能）のヘルパー関数を導入し、読み込みと追記を1行で完結させるアーキテクチャへ進化したこと」を追記する。

#### 【絶対遵守ルール (Constraints)】
*   **動作の維持**: 本タスクは整理とドキュメント更新のみであり、ロジックそのものの変更は行わないこと。タスク完了後に必ずビルドが通る状態を維持すること。
