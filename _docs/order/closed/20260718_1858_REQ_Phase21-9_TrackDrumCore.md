...
### 作業指示書 REQ: Phase 21-9 Hotfix 3 : プレイリスト切り替えに伴うテキスト残像の救済
*  D:\ozlab\oztone\PROJECT_CONSTITUTION.md
*  D:\ozlab\oztone\PROJECT_ARCHITECTURE.md
*  D:\ozlab\oztone\_docs\logs\20260718_2038_RES_Phase21-9_RelativeDrumCore.md

#### 【作業手順（厳守事項）】
1. 本プロンプトは、プレイリスト切り替え時に「前のプレイリストの曲情報」がオンデマンドで取得できなくなる問題を解決するための Hotfix の「実装実行」である。直ちに以下の【実装要件】に従ってコードの修正を実行すること。
2. 作業完了後、既存の作業レポート（20260718_2038_RES_Phase21-9_RelativeDrumCore.md）の末尾に「HOTFIX 3: プレイリスト切り替えに伴うテキスト残像の救済」を追加し、詳細作業内容を追記すること。
3. チャットにて「テキストのOLD残像の退避と描画ロジックの修正が完了しました。プレイリスト切り替え時も正しい残像が流れるか確認してください！」と報告すること。

#### 【実装要件】
*   **要件1: WidgetContext と Renderer へのテキスト残像変数の追加**
    *   `src/WidgetContext.h` および `src/Renderer.h` / `Renderer.cpp` に、画像だけでなくテキストの残像を保持するための変数 `std::wstring oldTrackTitle;` と `std::wstring oldTrackArtist;` を追加する。
*   **要件2: 曲切り替え時のテキスト退避（バケツリレーの復活）**
    *   Application層（または Renderer の `SetDrumTarget` 等の適切な場所）で曲が切り替わる直前、現在の `trackTitle` と `trackArtist` の値を `oldTrackTitle`, `oldTrackArtist` へ退避（コピー）させる処理を実装する。
*   **要件3: Widget_TrackInfo 描画ループにおけるハイブリッド描画の適用**
    *   `src/Widget_TrackInfo.cpp` の描画ループにおいて、直前までNOWだったOLDスロット（例: 進行方向に応じた `i == 1` または `i == -1` のスロット）を描画する際は、オンデマンド取得を行わず、退避された `ctx.oldTrackTitle` と `ctx.oldTrackArtist`（および `ctx.oldArtBitmap`）を描画するように条件分岐を修正する。
    *   目標スロット（`i == 0`）は現在の最新情報を描き、それ以外の中間スロット（大ジャンプ時のみ発生）は今まで通りオンデマンドでプレイリストを直参照するハイブリッド方式とする。

#### 【絶対遵守ルール (Constraints)】
*   **大ジャンプ時のオンデマンドの維持**: OLDスロットと目標スロット以外の中間スロット（同一プレイリスト内でジャンプした際の間の曲）については、引き続き `getTrackMetadata` を用いたオンデマンド描画を維持すること。
...
#### 【作業手順（厳守事項）】
1. 本プロンプトは、プレイリスト切り替え時に「前のプレイリストの曲情報」がオンデマンドで取得できなくなる問題を解決するための Hotfix の「実装実行」である。直ちに以下の【実装要件】に従ってコードの修正を実行すること。
2. 作業完了後、既存の作業レポート（20260718_2038_RES_Phase21-9_RelativeDrumCore.md）の末尾に、既存実装Hotfix追記対応時テンプレート（RES(AddHotfix)_template.md）を用いて作業の詳細を追記すること。
3. チャットにて「テキストのOLD残像の退避と描画ロジックの修正が完了しました。プレイリスト切り替え時も正しい残像が流れるか確認してください！」と報告すること。
