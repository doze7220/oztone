...
#### 【実装要件】
各UIセクションに分散・重複している「影」「フォント」「注視カラー」「ホバー/離脱ディレイ」のパラメータをパージし、新設する `[UI_Common_Parm]` セクションに一元管理するリファクタリングを行う。
...
*   **要件3: C++ 描画側への共通パラメータの適用**
    ...
    *   アクティブ色・ホバー色はすべて `FocusColor` を基準とし、OSDやチートシートのフォントは `OsdFontFamily` を参照するようにする。
