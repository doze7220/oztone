...
##### 作業指示書 REQ: Phase 20-2 Task 5: PROJECT_ARCHITECTURE.mdの更新 (実装実行)
以下のプロジェクトルールと開発資料、実装計画兼実装レポートを熟読すること。
*  D:\ozlab\oztone\PROJECT_CONSTITUTION.md
*  D:\ozlab\oztone\PROJECT_ARCHITECTURE.md
*  D:\ozlab\oztone\_docs\order\20260712_0913_REQ_Phase20-2_ApplicationRefactoring.md
...
###### 【作業手順（厳守事項）】
1. 本プロンプトはリファクタリングの「実装実行」である。直ちに以下の【実装要件】に従ってドキュメントの修正を実行すること。
2. 作業完了後、既存の作業レポート（D:\ozlab\oztone\_docs\logs\20260712_0914_RES_Phase20-2_ApplicationRefactoring.md）の「タスク5」のチェックボックスを完了 `[x]` にし、詳細作業内容を追記すること。
3. チャットにて「タスク5（ドキュメント更新）が完了しました。Phase 20-2の全タスク完了です」と報告すること。

---

###### 【実装要件】
今回のリファクタリング内容に合わせて、`PROJECT_ARCHITECTURE.md` の「Application クラス」の解説セクションを最新の構造に更新する。

*   **1. `PROJECT_ARCHITECTURE.md` の修正**
    *   **`Initialize` の説明更新**: 内部でコールバックの登録を行っていた記述を改め、「`SetupCallbacks` を呼び出して各種コールバックを登録する」という内容に修正すること。
    *   **新設メソッドの追記**: 以下のメソッドの説明を Application クラスのリファレンスに追加すること。
        *   `SetupCallbacks`: ウィンドウに対する各種マウス・キーボード等のコールバック登録を一元管理する。
        *   `PlayCurrentTrack`: メタデータの自己修復、AudioPlayerによる再生開始、次曲の先読み（PrefetchNextTrack）という再生開始の共通3点セットを集約して実行する（フェイルセーフのためboolを返す）。
    *   **ラムダ分離の追記**: プレイリスト操作等の巨大だったコールバックラムダ式が、`OnPlaylistClicked`、`OnPlaylistDoubleClicked`、`OnPlaylistToolbarClicked` などの独立したメンバ関数へ分離・整理された旨を、関連する説明部分（「プレイリスト連携」など）へ追記・修正すること。
