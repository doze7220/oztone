...
### 作業指示書 REQ: Hotfix Phase 22-3 アーキテクチャ資料の更新
*  D:\ozlab\oztone\PROJECT_CONSTITUTION.md
*  D:\ozlab\oztone\PROJECT_ARCHITECTURE.md
*  D:\ozlab\oztone\_docs\logs\20260719_1253_RES_Phase22-3_ThumbCacheLRU.md

#### 【作業手順（厳守事項）】
1. 本プロンプトはサムネイルエンジン（Phase 22-3）実装に伴うアーキテクチャ資料の更新である。直ちに以下の【実装要件】に従ってドキュメントの修正を実行すること。
2. 作業完了後、実装した内容と要件を照らし合わせ、反映漏れがないか厳密に自己監査を行うこと。
3. 監査完了後、既存の作業レポート（20260719_1253_RES_Phase22-3_ThumbCacheLRU.md）の末尾に、HOTFIXとして作業内容を追記すること。
4. チャットにて「アーキテクチャ資料の更新が完了しました」と報告すること。

#### 【実装要件】
`PROJECT_ARCHITECTURE.md` を更新し、Phase 22-3 で実装したデータベースのオンデマンド読み出しとLRUキャッシュ機構の仕様をクラスリファレンスへ反映する。極限までスリム化された資料の目的に従い、メソッドの詳細な引数などは書かず、責務と役割の概要のみを追記すること。

*   **要件1: クラスリファレンスの更新**
    *   `ThumbnailDatabase` クラスの説明に、パックファイルから必要な時にのみJPEGバイナリを吸い出してWICデコードする「オンデマンド読み出し機能」と、VRAM使用量を管理して古い画像を破棄する「LRUキャッシュ機構」が備わっている旨を追記すること。
