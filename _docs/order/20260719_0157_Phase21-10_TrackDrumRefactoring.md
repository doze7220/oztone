...
### 作業指示書 REQ: Phase 21-10 Task 6 & 7 : ビルド設定の更新とドキュメント反映
*  D:\ozlab\oztone\PROJECT_CONSTITUTION.md
*  D:\ozlab\oztone\PROJECT_ARCHITECTURE.md
*  D:\ozlab\oztone\_docs\logs\20260719_0203_RES_Phase21-10_TrackDrumRefactoring.md

#### 【作業手順（厳守事項）】
1. 本プロンプトはCMakeLists.txtとアーキテクチャ資料の更新である。直ちに以下の【実装要件】に従ってファイルの修正を実行すること。
2. 作業完了後、既存の作業レポート（20260719_0203_RES_Phase21-10_TrackDrumRefactoring.md）の「タスク6」および「タスク7」のチェックボックスを完了 [x] にし、詳細作業内容を追記すること。
3. チャットにて「Phase 21-10の全タスクが完了しました。ビルド・動作確認をお願いします」と報告すること。

#### 【実装要件】
分離した `TrackDrum` クラスをビルド対象に含め、プロジェクトの全体仕様書へアーキテクチャの変更を反映させる。

*   **要件1: CMakeLists.txt の更新 (タスク6)**
    *   `CMakeLists.txt` のソースファイルリスト (`SOURCES` 等) に `src/Renderer_TrackDrum.cpp` を追加する。
    *   ヘッダファイルリスト (`HEADERS` 等) に `src/Renderer_TrackDrum.h` を追加する。
*   **要件2: PROJECT_ARCHITECTURE.md の更新 (タスク7)**
    *   `PROJECT_ARCHITECTURE.md` の「5. 実装済みクラス・関数リファレンス」の `Renderer` クラスの項目内に、物理分割ファイルとして `Renderer_TrackDrum.cpp: ドラムアニメーションの物理演算と状態管理（TrackDrumクラス）` を追記する。
    *   合わせて、ドラムの機能が `Renderer` から切り離され、部品としてカプセル化された旨を適切な箇所に明記する。

#### 【絶対遵守ルール (Constraints)】
*   **ソースコードの変更禁止**: 本タスクでは `.cpp` や `.h` などのソースコード本体には一切触れないこと。
