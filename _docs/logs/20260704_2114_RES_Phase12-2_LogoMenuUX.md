...
## 6. Hotfix: プレイリスト展開時のホバー判定バグ修正 (Phase 12-2)

プレイリストが左配置されている際に、ロゴ・拡張メニュー領域へマウスが移動すると意図せずプレイリストが閉じてしまうバグを修正した。

**修正内容:**
* Window::IsInPlaylistRegion において、ロゴ本体および拡張メニュー領域の干渉排除ロジックを修正。
* プレイリストが展開中 (m_isPlaylistHovered == true) の場合は干渉排除ロジックを無効化し、プレイリストのホバー判定を優先するように条件分岐 (!m_isPlaylistHovered) を IsInLogoRegion による判定と Y座標による判定の両方に追加した。
* これにより、展開状態ではロゴ領域に重なっていてもホバーが維持され、非展開状態の場合のみメニュー展開との干渉を避ける仕様とした。
