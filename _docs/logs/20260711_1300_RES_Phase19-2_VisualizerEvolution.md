...
- メタデータとFFT解析のフラグ分離 (Phase 19-2 Hotfix)
  - 原因: `TrackMetadata` の `isLoaded` フラグが、「タグ情報の解析完了」と「FFT波形スキャンの完了」の両方の状態を兼用していたため、即時再生時のタグ自己修復処理によってフラグが `true` になると、裏スレッドのFFTスキャンが「解析完了」と誤認してスキップされてしまうバグが発生していた。
  - 対応:
    - `isLoaded` フラグを廃止し、タグ情報用の `isMetaLoaded` とFFT解析用の `isFFTLoaded` の2つの独立したフラグに分離した。
    - メタデータのみの更新時（`UpdateTrackMetadataIfNeeded`等）は `isMetaLoaded` のみを `true` にし、FFT波形解析を含む完全な更新時（`ParseThreadFunc`の裏スキャン等）に両方のフラグを `true` にするよう更新ロジックを分離した。
    - バックグラウンドの解析対象を取得する処理のスキップ判定において、評価するフラグをメタデータではなく `isFFTLoaded` に変更し、タグ情報が先行して取得されていてもFFTスキャンが未完了であれば確実に裏スレッドで解析対象となるよう修正した。

- 解析スレッドのファイル競合と依存関係の分離 (Phase 19-2 Hotfix)
  - 原因: 即時再生される1曲目において、メインスレッドとバックグラウンドスレッドの両方が同時に `TagManager::Load` を呼び出そうとしてファイルロック（共有アクセス違反）が発生し、裏スレッド側がタグ読み込みに失敗していた。さらに、裏スレッドの処理がタグ解析の成否に依存しており、読み込みに失敗するとFFTスキャンを含む以降の処理を中断してしまっていたため、1曲目がFFT未解析のままになるバグが発生していた。
  - 対応: `src/Application.cpp` の `ParseThreadFunc` 内の処理を、タグ解析（`isMetaLoaded`）とFFTスキャン（`isFFTLoaded`）の2つの完全に独立したステップに分離した。タグ解析に失敗（または例外発生）してもエラーを捕捉して中断せずに次のFFT解析へ進むように変更。これにより、メインスレッドが先行してファイルをロックしていても、裏スレッドは安全にタグ読み込みを処理・スキップしてFFTスキャンのみを確実に実行できるようになった。

- ビジュアライザの責務分離と256ビン正規化のデータ指向アーキテクチャ改修 (Phase 19-2 Hotfix)
  - 原因: 個別のビジュアライザ（子）の中で生のFFTデータに対する対数マッピングやEQ処理を行っていたため、曲ごとの最大周波数の違いによって描画側の配列アクセスや計算が破綻するバグが発生していた。
  - 対応: 
    - `src/Visualizer.cpp` (親ファサード) にて、渡された生スペクトルデータから 0 〜 `maxFrequency` の範囲を対数スケールでサンプリングし、5バンドEQと `std::clamp` を適用した256個の「完成済み配列 (`processedSpectrum`)」を生成して子へ渡すデータ指向アーキテクチャへ改修した。
    - `src/Visualizer_PrismBeat.cpp` および `src/Visualizer_HaloDust.cpp` (子プラグイン) にて、対数計算やEQゲイン取得などの複雑な前処理をすべて削除し、親から渡された常にサイズ256のデータを画面幅やバンド数へ純粋に線形マッピングして描画するだけのロジックへリファクタリングした。

- ビジュアライザ対数マッピングの下限修正 (Phase 19-2 Hotfix)
  - 原因: `Visualizer.cpp` における対数マッピングの計算において、下限値が極端に低く設定されていたため、低音域のビジュアルの立ち上がりが遅く、左側の複数のビンが20Hz以下の無音帯域を参照してしまい動かなくなるバグが発生していた。
  - 対応: 256ビン生成ループの前処理において、対数マッピングの下限インデックス (`minIndex`) を `4.0f` (約20Hz相当) に明示的に設定し、インデックス `4.0f` から有効な最高周波数までの範囲を綺麗に対数分割して生の振幅値を抽出するよう計算ロジックを修正した。
