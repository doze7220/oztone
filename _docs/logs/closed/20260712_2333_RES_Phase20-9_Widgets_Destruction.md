...
[x] タスク7: OsdWidget の分離
    ...
    - インクルード修正と `CMakeLists.txt` への追加を行う。
[x] タスク8: 旧 Widgets.h / .cpp の削除とクリーンアップ
...
### タスク8: 旧 Widgets.h / .cpp の削除とクリーンアップ
    - `src/Widgets.h` と `src/Widgets.cpp` をファイルシステムから物理削除した。
    - `CMakeLists.txt` から `src/Widgets.h` と `src/Widgets.cpp` のエントリを削除した。
    - `src/Renderer.h` 内に残存していた `#include "Widgets.h"` を `#include "Widget.h"` に置換し、ビルドエラーを解決した。
    - ビルドを実行し、正常に完了することを確認した。

### タスク9: ドキュメントの更新
    - `PROJECT_ARCHITECTURE.md` を更新し、旧ファイル `Widgets.h/cpp` に関する記述を `Widget_*.h/cpp` のプレフィックス命名規則に基づいた新しいファイル群の名称に修正した。
    - `OsdWidget` についても `Widget_Osd.h/cpp` と名称を合わせた記述に更新した。
