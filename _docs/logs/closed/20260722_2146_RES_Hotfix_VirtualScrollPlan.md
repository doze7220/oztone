...
### タスク8: Widget層 - アフォーダンス描画
    ...
    **【作業内容】**
    - `src/WidgetContext.h` にホバー状態を伝達するための `isTrackInfoHovered` を追加し、`Renderer` クラスの各更新・描画系メソッドの引数として渡すようにパイプラインを構築した。
    - `src/Application_Render.cpp` のメインループ内で `m_window.IsTrackInfoHovered()` を取得し、`Renderer::UpdateAnimation` および `Renderer::Render` へ引き渡すよう修正した。
    - `src/Widget_TrackInfo.h` および `src/Widget_TrackInfo.cpp` にて、ツールチップ描画用のリソース（ジオメトリやブラシ類）の生成・解放処理を追加し、`UpdateAnimation` でホバー状態に応じた非対称フェード（フェードイン・フェードアウト）のパラメーターを更新するロジックを実装した。
    - `Widget_TrackInfo::Draw` において、`m_hoverAlpha > 0.0f` の場合にアルバムアートの中心を基準とし、`ConfigManager` から取得したオフセット位置へ `WidgetCommon::DrawMouseScrollTooltip` を呼び出してツールチップを描画する処理を実装した。
### タスク9: ドキュメントの更新
    **【対象ファイル】**
    - `PROJECT_ARCHITECTURE.md`
    **【作業内容】**
    - `PROJECT_ARCHITECTURE.md` のUI/ビジュアル仕様方針に「仮想スクロールと再生確定」の項目を追記し、全体仕様を更新した。
    - `Window` クラスの役割にTrackInfo領域のホバー判定と、アキュムレータによるホイール入力の初動重み付けを追記した。
    - `Application` クラスに仮想スクロール時の待機タイマー管理、自動曲送りブロック、ファイルロックを回避したドラムの先行発注および再生確定ロジックを追記した。
    - `TrackInfoWidget` の役割に、共通ユーティリティ（WidgetCommon）を用いたホバー時のアフォーダンス（吹き出しツールチップ）描画を追記した。
