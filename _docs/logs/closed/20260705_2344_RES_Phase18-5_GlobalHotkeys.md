...
## HOTFIX
### 原因
チートシート（ネオン表示）のキー名と機能名が結合された1つの文字列として描画されており、等幅フォントでない場合に列が揃わず視認性が低下していた。また、描画位置の微調整がハードコードされていたため、カスタマイズが困難であった。

### 対応内容
* ConfigManager に [Layout_GlobalHotkeys] セクションのパラメータ (GlowOpacity, KeyColumnOffset, ActionColumnOffset) を追加し、カスタマイズ可能とした。
* GlobalHotkeysWidget において、GetActionName メソッドを新設し、アクションIDから機能名文字列を取得するロジックを分離した。
* 文字列をキー名と機能名に分離して保持・取得するよう改修し、Draw メソッド内で KeyColumnOffset および ActionColumnOffset を起点として左寄せ (DWRITE_TEXT_ALIGNMENT_LEADING) で2カラムの表組みレイアウトとして描画するようアーキテクチャを変更した。
