...
### タスク2: ConfigManagerへの設定追加
    ...
    **【作業内容】**
    - `ConfigManager.h`に`TooltipOffsetX`および`TooltipOffsetY`のゲッターとメンバ変数を追加した。また、OSDの有効無効を制御する`EnableOSD`のメンバ変数、ゲッター、セッターを追加した。
    - `ConfigManager_Playback.cpp`の`[Layout_NowPlaying]`セクション読込処理にて、ツールチップのパラメータのパース処理を実装した。
    - `ConfigManager_System.cpp`の`[Layout_OSD]`セクション読込処理にて、`EnableOSD`のパース処理とセッターの実装を追加した。
    - `ConfigManager_DefaultIni.h`の初期値にツールチップ位置微調整用デフォルト値(`TooltipOffsetX=-10.0`, `TooltipOffsetY=-20.0`)と、OSD有効化デフォルト値(`EnableOSD=1`)を追記し、単一情報源(SSOT)の原則を維持した。
### タスク3: Window層 - TrackInfo領域のホバー判定追加 (WM_MOUSEMOVE 等の対応)
    **【対象ファイル】**
    - 
    **【作業内容】**
