...
*   **各具象 Widget 実装 (`Widgets.h/cpp`)**
    ...
    *   `PlaybackControlsWidget`, `VolumeControlWidget`: 画面下部ホバー時に白ベクターアイコンでフェードインする再生コントロール（5ボタン構成）および音量コントロールの描画。`PlaybackControlsWidget` では各ボタンごとにホバー状態を追跡し、非対称フェードによる色合成(`HoverIconColor`)を適用している。また、10秒スキップ用の「≪」「≫」ボタンについて、`ConfigManager` から取得した `SkipIconPoints` 等の座標パラメータやテキストパラメータを用いて動的にポリゴン（面）や影付きインジケーターを生成・描画し、鋭利なシルエットとINIからのカスタマイズ性を両立している。
