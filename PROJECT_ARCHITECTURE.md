...
Direct3D 11 / Direct2D / DirectComposition を用いたハイブリッド描画エンジン。内部ロジックを論理ピクセル（96DPI基準）に統一し、描画の根元で `SetTransform` を用いて一括DPIスケールするアーキテクチャ。背景アートとビジュアライザを自前で描画し、それ以外のUI要素は全て `IWidget` 実装クラス群に委譲する。ドラムアニメーション（フリップサイクル）のステートマシンを内包し、コールバック・イベント駆動によるデータ層とのデカップリングおよび「究極のカプセル化」を実現している。背景アート描画はドラムスロットの同期から完全にパージ・分離され、専用の `m_backgroundArtBitmap` によって曲決定と同時に即時描画される独立したライフサイクルを持つ。
**物理分割ファイル:**
