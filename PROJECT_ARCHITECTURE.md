...
*   **UI/ビジュアル仕様方針**:
    ...
    *   下部：シークバー、およびマウスをホバーさせた時のみフェードインして浮かび上がる再生コントロール（前の曲、10秒戻る、再生/一時停止、10秒進む、次の曲の5ボタン構成）と音量コントロール（スピーカーアイコンとパーセンテージ表示）。ホバー領域に入るとシークバー本体は視認性向上のため50%減光する。また、マウスホイールで直感的な音量調整が可能。
    ...
    *   **設定/システムトレイアイコン**: 設定はシステムトレイアイコンに集約させ、コンフィグウィンドウは限界まで作成しない。システムトレイアイコンからは、直感的なアクセスが求められる再生コントロール（再生/一時停止、前・次の曲）や音量調整サブメニューへのアクセスを最優先で提供する。画面固定（移動ロック）、INIファイルのリセット機能などの利用頻度が低い高度な設定項目は「詳細設定 (Advanced)」サブメニューにネストさせて整理する。画面サイズやレイアウトなどは、INIファイルを直接編集することでカスタマイズ可能とする。
...
*   **`void OnFilesDropped(const std::vector<std::wstring>& paths)`**
    ...
    *   また、実験的対応として動画コンテナ形式（.mp4, .m4a）も、マジックナンバー検証をスキップして通過させる。この暫定処理はコード上で `// [EXPERIMENTAL] MP4/M4A Support` というコメントにて隔離されている。
...
*   **`void SwitchPlaylist(const std::wstring& filepath)`**
    *   既存のプレイリストファイルへ動的に切り替える。ただし、指定されたファイルパスが現在読み込まれているプレイリストと同一である場合は、再生を維持したままリスト一覧モードを終了するだけで再ロードを防止する。切り替え時は、設定ファイル(`OZtone.ini`)のパスを更新した後、現在の再生を停止・キューとUIをリセットし、メモリ上のプレイリストをクリア（`PlaylistManager::Clear`）してから指定されたファイルをロードする。ファイルのロード後は、自動で再生と先読みを開始するとともに、未解析のトラックがある場合はバックグラウンド解析キューへ追加し、専用スレッドを起床させて非同期でのタグ解析を開始する。
...
*   **各具象 Widget 実装 (`Widgets.h/cpp`)**
    ...
    *   `LogoMenuWidget` (`src/LogoMenuWidget.h/cpp`): アプリアイコンホバー時に右側へアニメーション進行度に応じたイージングでスライド展開する「ロゴ拡張メニュー」の描画。配列ベースで管理されたメニューアイコンの展開、ホバー時における背面グロー画像（`app_logo_back.png`）の合成、トグルOFF時（またはビジュアライザ無効時）の視覚フィードバック（半透明化）、およびビジュアライザ・背景表示アイコン等の右下のインジケーター等の縁取り描画を担当する。また、状態（プレイリスト左右配置等）やホバーアイコンに連動した動的なテキスト説明表示（シャドウ付き）を行う。
    ...
    *   `PlaybackControlsWidget`, `VolumeControlWidget`: 画面下部ホバー時に白ベクターアイコンでフェードインする再生コントロール（5ボタン構成）および音量コントロールの描画。`PlaybackControlsWidget` では10秒スキップ用の「≪」「≫」ボタンについて、`ConfigManager` から取得した `SkipIconPoints` 等の座標パラメータやテキストパラメータを用いて動的にポリゴン（面）や影付きインジケーターを生成・描画し、鋭利なシルエットとINIからのカスタマイズ性を両立している。
    *   `GlobalHotkeysWidget`: 設定で有効時のみ、右上にグローバルホットキー設定をネオン文字で描画するチートシート機能。`ConfigManager` から取得した `[Layout_GlobalHotkeys]` のパラメータ（フォント、コア色、グロー色、ドロップシャドウ等）を用いて動的に発光表現を構築し、INIからのフルカスタマイズに対応している。`PlaylistWidget` の奥へ沈み込むZ-Orderで配置される。
    *   `PlaylistWidget` (`src/PlaylistWidget.h/cpp`): INI設定(`PlaylistPosition`)に応じ、画面左端または右端のホバー時にスライドインするシャッフル再生リストと「引き出しグリップ」の描画。グリップの矢印の向きも左右配置に応じて動的に反転する。`Application` から渡されたモード情報（`isPlaylistListViewMode`）に基づき、曲一覧またはプレイリスト一覧を切り替えて描画し、上部に固定ツールバー（上段にアイコン、下段に日本語説明テキスト）を描画する純粋な描画責務を持つ。ツールバーには「表示固定（ピン留め）」機能のトグルボタンも備えられ、現在の状態（自動格納／画面固定）に応じたサブアイコン（🔓/🔒）およびツールチップを描画する。曲一覧モードの非ホバー時には、設定から取得した現在のプレイリストパスを直接パース（拡張子を除外）してツールバーにプレイリスト名を常時表示する。自身でスライドインアニメーションやスクロール制御、限界値クランプ、各テキストの描画を担当し、対象曲のメタデータが解析済み（`isLoaded == true`）の場合は取得した曲名・アーティスト・再生時間を使用してリッチに描画、未解析の場合はファイル名から推測したタイトルでのフォールバック描画を行う。プレイリスト一覧モード時においても、`Application`側で非同期・軽量にパースされ `WidgetContext` 経由でキャッシュとして供給される `PlaylistSummary` （表示名、曲数、総時間）を利用し、上段にプレイリスト名、下段に曲数と総再生時間を配置するリッチな2行スタイルで描画を行う。また、ピン留めが有効な状態では、格納を想起させる引き出しグリップの矢印描画をスキップしアフォーダンスを調整している。
