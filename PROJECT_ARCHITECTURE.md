...
*   **UI/ビジュアル仕様方針**:
    ...
    *   **ウィンドウのドラッグ移動**: 特定のUI要素がない背景部分全体をドラッグすることでウィンドウを移動できる。また、SHIFTキーを押しながらドラッグすることで、ウィンドウではなく背景アルバムアート自体を移動（フレーミング調整）させることが可能。
    ...
    *   **隠れるUIの共通離脱ディレイ**: ロゴ拡張メニュー、プレイリスト、再生・音量コントロール等、「マウスホバー時のみ現れるUI」はすべて共通して「離脱ディレイ（猶予時間）」を持つ。ホバー状態から外れた場合でも即座に格納せず、設定されたディレイ時間（例：3.0秒）の間は展開状態を維持することで、操作中の意図しない格納を防ぎ、直感的な操作感と統一されたUXを提供する。
...
*   **`void ForceRender()`**
    *   現在時間やスペクトルデータの取得、UIホバー状態（プレイリストホバー状態も含む）の収集を行い、`Renderer::UpdateAnimation` と `Renderer::UpdateTextLayouts` を呼び出してアニメーション状態および変動テキストのレイアウトキャッシュを更新した後、強制的に1フレーム分の描画処理 (`Renderer::Render`) を実行する。この際、`PlaylistManager` から現在のインデックス、総曲数、シャッフルメタデータリスト一覧を取得してRendererに渡し、状態を連携する。
    ...
    *   **背景フレーミングの永続化とクランプ管理**: `Window` 側で検知されたSHIFT＋ドラッグやホイールによる背景画像のオフセット移動・ズーム情報をコールバックで受け取り、`Renderer` にクランプ処理を委譲しつつ、メモリ上の `PlaylistManager` (`TrackMetadata`) にリアルタイムに反映する。ディスクI/O負荷を低減するため、操作のたびにTSVファイルへ保存するのではなく、曲が切り替わる直前（自動進行、手動スキップ、ジャンプ再生）やプレイリストの破棄直前、およびアプリ終了時にのみ一括でセーブを行うよう最適化されている。
*   **`void ClearPlaylist()`**
...
*   **`LRESULT WindowProc(HWND, UINT, WPARAM, LPARAM)`**
    ...
    *   **背景フレーミングの入力フック**: 背景の何もない領域において、SHIFTキー（`MK_SHIFT`）を押下しながらの左ボタンドラッグ（移動）およびマウスホイール（拡大縮小）を検知し、専用のフレーミング操作としてフックする。また、マウス中ボタンクリックを検知してフレーミングのリセット操作として処理する。これらの操作は専用のコールバックを介して `Application` へ伝達される。
...
内部のリストデータは `TrackMetadata` 構造体（ファイルパス、曲名、アーティスト名、再生時間、解析済みフラグ、**アートのフレーミング情報(オフセット・スケール)** を保持）で管理され、`std::mutex` によりマルチスレッドからの安全な排他制御アクセスが保証されている。フレーミング情報は永続化され、曲ごとにカスタマイズされたアートの位置や拡大率を維持する。
