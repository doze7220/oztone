...
*   **UI/ビジュアル仕様方針**:
    ...
    *   ロゴ拡張メニューの機能：アプリ終了、再生中の曲削除、プレイリスト固定トグル、ビジュアライザ3ステート切替（PrismBeat, Halo Dust, OFF）、シャッフル切替、プレイリスト左右配置切替。ホバー時には背景画像のフェードおよびグロー効果が付与され、上部に機能説明テキスト（英語大文字、影付き）が動的に表示される。
