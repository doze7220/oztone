...
*   **`LRESULT WindowProc(HWND, UINT, WPARAM, LPARAM)`**
    ...
    *   **WM_MOUSEMOVE等のホバー制御**: `WM_MOUSEMOVE`、`WM_MOUSELEAVE` 等でマウス位置を追跡し、ロゴ領域、ロゴ拡張メニュー領域(`IsInLogoMenuRegion`)、再生コントロール領域へのホバー状態（フェードやスライド展開用）を管理する。
    *   **プレイリストのホバー制御と排他制御**: `WM_MOUSEMOVE` 時に `IsInPlaylistRegion` を用いてプレイリスト領域へのホバーを判定する。この判定ではINI設定(`PlaylistPosition`)に基づき左右どちらからの展開かを動的に判定する。また、左配置設定時に左上のアプリアイコンおよびロゴ拡張メニュー（ドラッグ領域・展開領域）と干渉しないよう、ロゴ周りの判定が優先される。プレイリスト展開中は、ホバー判定幅を `PlaylistHoverWidth` からリスト全体の幅に動的拡張するとともに、画面下部UIとの干渉回避用Y座標制限を無効化し、リスト全域でホバー状態を維持する。また、プレイリストホバー時は、背後の再生コントロールや音量調整の判定をスキップする排他制御を行う。`WM_MOUSEWHEEL` ではプレイリスト用スクロールコールバックを呼び出し、`WM_LBUTTONDOWN` では各クリックハンドリング（ロゴ拡張メニュー項目やプレイリスト曲等）を呼び出して処理へ委譲する（この際も背後の他のUI判定より優先して早期リターンし誤動作を防ぐ）。
...
*   **各具象 Widget 実装 (`Widgets.h/cpp`)**
    ...
    *   `LogoMenuWidget`: アプリアイコンホバー時に右側へスライド展開する「ロゴ拡張メニュー」の描画。絵文字等を用いた白単色アイコンの展開、トグルOFF時の斜線描画、イージングに同期したタイピング風テキスト演出（`IDWriteTextLayout1::SetCharacterSpacing` による文字間隔制御等）を担当する。
