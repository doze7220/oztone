...
**【特記事項】現在はPhase 23にて FileManager リファクタリングが完了し、FileManager が TagLib の機能を吸収してファイルI/Oおよびメタデータ抽出の単一の門番として稼働している。旧来の TagManager は削除された。**

本資料は、AIがタスクを実行する際の「全体マップ（見取り図）」および「インデックス（索引）」として機能するものである。AIがどの責務を修正する際にどのファイル（個室）を開くべきかを示す道標に特化させる。
したがって、直接コードを読めば瞬時に理解できる詳細なメソッドの内部ロジック、関数の引数、設定パラメータの全列挙などはAIのコンテキストを圧迫する「ノイズ」となるため、本資料には記載しない。実装された項目をここへ随時追記・更新していくこと。
...
*   **UI/ビジュアル仕様方針**:
    ...
    5. **ジョグダイヤリング (Jog Dialing)**: 左下のTrackInfo領域はマウスホバー時のアフォーダンス（ツールチップ）とホイールによるパラパラめくりをサポートし、一定時間入力が停止した段階で対象曲へのジャンプと再生が確定する。ホイール入力とアキュムレータは `Window` クラス、待機タイマーと再生確定ロジックは `Application` クラスが担当している。ジョグダイヤリングやNext/Prev等の能動的な操作時のみプレイリストが自動センタリングされ、自動再生時はスクロール位置を維持する設計となっている。
### 2. 禁止事項・ルール (AI向け)
...
5.  **状態管理と画像ロード（3スロット式リングバッファ）**:
    *   3つの `DrumSlot` 構造体を循環させる「3スロットのリングバッファ」により状態を管理する。
    *   画面外に押し出されたスロットを利用し、「0ぴったり（整数位置到達時）」のタイミングで次に表示される曲のデータを先行注入（Pre-fill）するスクロール機構となっている。
    *   外部（ApplicationやWidget）から `DrumSlot` への直接操作は禁止されており、データの上書きは `Drum` エンジン内部の先行注入時にのみ行われる。
    *   **動的間引きスクロール（ダイナミックスケール）の仕様**:
        ...
        *   スクロール中の画面外Pre-fill時に、このステップ幅を乗算して「中間に位置する間引かれた曲」を動的に算出し、差し込みながら目標曲への同期着地を実現している。
6.  **コールバック駆動とイベント駆動のステートマシン**:
    *   アニメーションは外部から「相対距離」と「データ取得用コールバック」、「完了時コールバック」を渡す **コールバック駆動** である。
    *   アニメーションの物理演算層（`UpdateAnimation`）は距離を計算し、1スロット分進んだという通知のみをエンジン本体へ行う **イベント駆動** である。
    *   通知を受けたエンジン本体が、目標到達前ならコールバック経由で必要なメタデータを取得してフリップし、到達していれば完了通知を出す。
7.  **描画更新層（UpdateTextLayouts）**:
    *   テキストレイアウトの更新処理は、`DrumSlot` 内に保持されているデータを用いてキャッシュを再構築するのみであり、外部データからの毎フレーム上書き処理は行わない。
...
アプリケーション全体のライフサイクル、メインループ、再生制御、プレイリスト管理、ファイル入力処理を統括するマネージャ。スリープ時のオーディオエンジンのサスペンド・レジューム管理や、オーディオエンジンの異常停止を検知し自己修復するウォッチドッグ機構を内包。メインループ内ではプレイリストファイル群のスナップショット監視（毎秒ポーリング）を行い、アクティブファイルの消失時には安全な空状態へ移行するフェイルセーフ機構を持つ。さらに、ThumbnailDatabaseとThumbCacherを仲介する司令塔としての責務を持ち、単方向依存でサムネイル生成の発注管理を行う。仮想スクロール時は待機タイマーによる状態管理を行い、自動曲送りをブロックしつつ、ファイルロックを回避したドラムアニメーションの先行発注と再生確定ロジックを制御する。
**物理分割ファイル:**
...
Win32 APIのウィンドウ生成・破棄、メッセージディスパッチ、マウス入力・ホバー判定、トレイアイコン、ドラッグ＆ドロップ、グローバルホットキー等のOS連携処理を隠蔽・カプセル化するクラス。OSの電源イベント（スリープ移行・復帰）のフック。また、TrackInfo領域のホバー判定と、アキュムレータを用いたホイール入力の初動重み付け（初動の重さと連続スクロールの軽快さの切り替え）を担う。
**物理分割ファイル:**
*   `Window.cpp`: ウィンドウプロシージャ等のエントリポイント
*   `Window_Proc.cpp`: メッセージディスパッチの基盤
...
#### `ConfigManager` クラス (src/Config/ConfigManager.h)
アプリケーションの設定（`OZtone.ini`）の読み込み・書き出しを管理する。全設定の初期値は `ConfigManager_DefaultIni.h` の `DEFAULT_INI_CONTENT` にSSOT化されている。初期ファイル生成時は `WritePrivateProfileStringW` による後続の書き込みとの整合性を保つため、**UTF-16 LE (BOM付き)** 形式でバイナリ出力される。（※以前保持していたプレイリストディレクトリのスナップショット監視機能は `FileManager` および `Application` へ移管され、純粋な設定コンテナとして純化された）
**Phase 24-1 のアーキテクチャ変更により、「1 INIセクション ＝ 1 構造体 ＝ 1 cppファイル」の形に完全にモジュール化された。**
**物理分割ファイル (src/Config/ 配下):**
*   `ConfigManager.cpp`: デフォルト設定文字列とファイルI/Oコア処理、各種設定保存処理
*   `ConfigManager_DefaultIni.h`: 全設定のSSOTとなるデフォルトINI設定値
*   `Config_System.h/cpp` 等: `System`, `Window`, `Audio`, `Layout_***`, `Visualizer_***` など、22個のINIセクション別パーサー・状態構造体モジュール

#### `Renderer` クラス (src/Renderer.h)
Direct3D 11 / Direct2D / DirectComposition を用いたハイブリッド描画エンジン。内部ロジックを論理ピクセル（96DPI基準）に統一し、描画の根元で `SetTransform` を用いて一括DPIスケールするアーキテクチャ。ビジュアライザを自前で描画し、それ以外のUI要素は全て `IWidget` 実装クラス群に委譲する。ドラムアニメーション（リングバッファ）のステートマシンは独立した部品として `TrackDrum` クラスにカプセル化され、機能が `Renderer` から分離されている。背景描画に関しては、背景の位置や拡大率などの状態を持たず、設定の参照処理も行わない。`BackgroundManager` が構築したレイヤー配列（`BackgroundLayer`）を先頭から順次描画し、レイヤーグループのPush/Popコマンドを受動的に解釈して不透明度を一括適用するペインターとして機能する。
**物理分割ファイル:**
...
**各具象Widget:**
*   `AppLogoWidget` (src/Widget_AppLogo.h/cpp): アプリアイコンの描画（非ホバー時の半透明白アイコンからホバー時のカラーアイコンへのクロスフェード対応）
*   `LogoMenuWidget` (src/Widget_LogoMenu.h/cpp): アプリアイコンホバー時にスライド展開するロゴ拡張メニューの描画
*   `TrackInfoWidget` (src/Widget_TrackInfo.h/cpp): 左下のアルバムアート・曲名・アーティスト名の描画。データ層へは直接アクセスせず、コンテキストから渡される独立した3つのスロット (`drumSlots`) の情報を描画する。中間アニメーション時は半透明板のフォールバックを描画する。テキストレイアウト更新処理 (`UpdateTextLayouts`) は、スロット内データからのキャッシュ再構築のみを行う。ホバー時はアフォーダンスとして、吹き出しツールチップを描画する。
...
*   `PlaylistWidget` (src/Widget_Playlist.h/cpp): プレイリスト一覧・曲一覧のスライドイン描画およびツールバー。ジョグダイヤリングやNext/Prev等の能動的な操作時のみ自動センタリングされ、自動再生時はスクロール位置を維持する。以下に物理分割：
...
ファイルI/Oおよびメタデータ抽出を単一の窓口として引き受けるクラス。外部に対して TagLib などの存在を隠蔽する Adapter パターンとして機能する。軽量なテキストメタデータの抽出と、重い画像バイナリのオンデマンド抽出に責務を分割している。また、一時的なファイルロック競合を回避するリトライ機構を内部にカプセル化し、I/Oの責務を集約している。また、プレイリストディレクトリの走査（一覧取得）やスナップショット更新検知など、ファイルシステムへの直接的なアクセスロジックもすべて本クラスに集約・カプセル化されている。
...
タグ解析およびFFT波形事前スキャンをバックグラウンドスレッドで非同期実行し、完了データを `TrackDatabase` へ自律的に書き込むクラス。ファイルI/Oやタグ解析時の未知の例外を捕捉し、スレッドをクラッシュさせずに次の解析を継続する強固なフェイルセーフ機構を備える。

#### `ThumbnailManager` クラス (src/ThumbnailManager.h, cpp)
サムネイルに関するすべての処理を統括する視覚の司令塔（ファサード）クラス。内部に `ThumbCacher`（画像生成ワーカー）と `ThumbnailDatabase`（パックファイル管理・LRUキャッシュ）をカプセル化しており、外部に対するサムネイル操作の単一の窓口として機能する。

#### `ThumbnailDatabase` クラス (src/ThumbnailDatabase.h, cpp)
サムネイル画像のバイナリデータ（パックファイル）の読み書きと、LRUアルゴリズムに基づくオンメモリキャッシュ管理を行うデータベースクラス。`ThumbnailManager` の配下で稼働する内部コンポーネントであり、IDとファイルパスの紐付け管理を行う「純粋な辞書管理・窓口」として機能する。スレッドセーフなバイナリ追記機能とセクタ情報のメモリ同期機能に加え、必要な時にのみパックファイルからJPEGバイナリを吸い出してWICデコードする「オンデマンド読み出し機能」と、VRAM使用量を管理して古い画像を破棄する「LRUキャッシュ機構」を備える。
さらに、デストラクタにおけるタスクカウントを用いたスレッドの安全終了（待機）機構を備えており、アプリ終了時のクラッシュを防止する堅牢なライフサイクル管理を実現している。

#### `ThumbCacher` クラス (src/ThumbCacher.h, cpp)
バックグラウンドスレッドで非同期にサムネイル画像を生成・リサイズし、キューイングされたタスクを順次処理する専用のエンジンクラス。`ThumbnailManager` の配下で稼働する内部コンポーネントであり、外部からの発注（IDとパスのペア）に従ってサムネイルを生成するワーカーとして機能する。FileManager と連携し、非同期に画像バイナリを抽出・WICデコードしてサムネイルを生成・保存する。
画像データ破損等により生成・保存処理を完遂できなかった場合は、速やかにデータベースの発番情報をロールバック（破棄）し、次回のUI更新タイミングで再発注させる自己修復機構を持つ。

#### `BackgroundManager` クラス (src/BackgroundManager.h, cpp)
背景アートの保持、フレーミング状態（拡大率やオフセット）の管理、および描画レイヤーの構築を統括するクラス。Rendererから背景に関する状態と設定依存を引き継いでいる。FileManagerを用いた非同期画像抽出とWICデコードを専用のバックグラウンドスレッドで担当し、UIスレッドをブロックしない。
描画層に対しては、D2Dのレイヤーグループ（`PushLayer` / `PopLayer` に対応するマーカー）を用いた構造を動的構築して配列として渡し、Rendererに描画させる。これにより合成された背景全体に対して `BgOpacity` を一括適用する。クロスフェードは、OLD/NEWそれぞれ独立してフレーミングを保持しつつ、フェード率が1.0fに達した瞬間にNEWの画像と状態をOLDへスワップする機構となっている。
