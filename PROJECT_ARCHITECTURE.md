...
*   **各具象 Widget 実装 (`Widget_AppLogo.h/cpp`, `Widget_TrackInfo.h/cpp` 等)**
    ...
    *   `PlaylistWidget` (`src/Widget_Playlist.h/cpp`): INI設定(`PlaylistPosition`)に応じ、画面左端または右端のホバー時にスライドインするシャッフル再生リストと「引き出しグリップ」の描画。グリップの矢印の向きも左右配置に応じて動的に反転する。`Application` から渡されたモード情報（`isPlaylistListViewMode`）に基づき、曲一覧またはプレイリスト一覧を切り替えて描画し、上部に固定ツールバー（上段にアイコン、下段に日本語説明テキスト）を描画する純粋な描画責務を持つ。ツールバーには「表示固定（ピン留め）」機能のトグルボタンも備えられ、現在の状態（自動格納／画面固定）に応じたサブアイコン（🔓/🔒）およびツールチップを描画する。曲一覧モードの非ホバー時には、設定から取得した現在のプレイリストパスを直接パース（拡張子を除外）してツールバーにプレイリスト名を常時表示する。自身でスライドインアニメーションやスクロール制御、限界値クランプ、各テキストの描画を担当し、対象曲のメタデータが解析済み（`isLoaded == true`）の場合は取得した曲名・アーティスト・再生時間を使用してリッチに描画、未解析の場合はファイル名から推測したタイトルでのフォールバック描画を行う。また、描画ループ内でアイテムのホバー状態（`HoverItemColor`）および再生中状態（`PlayingItemColor`）に基づき、対象アイテムのみのアルファ値を非対称フェードで計算・合成することで、数千件のプレイリストでも高パフォーマンスに色遷移を実現している。プレイリスト一覧モード時においても、`Application`側で非同期・軽量にパースされ `WidgetContext` 経由でキャッシュとして供給される `PlaylistSummary` （表示名、曲数、総時間）を利用し、上段にプレイリスト名、下段に曲数と総再生時間を配置するリッチな2行スタイルで描画を行う。また、ピン留めが有効な状態では、格納を想起させる引き出しグリップの矢印描画をスキップしアフォーダンスを調整している。将来の「物理ファイル分割」および「全曲アルバムアート非同期サムネイル表示（LRUキャッシュ・非同期ロード）」という高度な機能拡張を見据えたアーキテクチャの整理（地盤固め）が行われている。巨大化していた描画ループ(`Draw()`)が完全に解体され、`DrawGrip`, `DrawBackground`, `DrawToolbar`, `DrawPlaylistItems`, `DrawPlaylistList`, `DrawTrackList` などの専用メソッドへと責務が細分化（目次化）された。各描画メソッドはメンバ変数への直接依存を極力排除し、外部から依存関係（コンテキストやレイアウト情報）を引数で受け取る独立した設計へとリファクタリングされており、物理的なファイル分割はまだ行われていないものの、データ生成と描画の分離（`TrackVisualData`構想など）を見据えた最強のクリーン地盤が完成している。
