...
### 3. プロジェクトフォルダ構造
現在のディレクトリ・ファイル構成は以下の通り（Phase 2 Step 2 完了時点）。
...
*   ウィンドウ表示フラグ (`GetShowTitleBar` 等) と、レイアウト情報 (`GetLogoX` 等) のゲッターを提供する。
...
*   **`bool Initialize(HWND hwnd, const ConfigManager& config)`**
    *   D3D11デバイス、DXGIスワップチェイン、D2D1ファクトリ等を作成。WICファクトリを初期化し、初期アセット（アプリアイコンなど）を読み込んで D2D1Bitmap を準備する。
...
*   **`void Render(bool isHovered)`**
    *   毎フレーム呼び出され、描画処理を実行。現在は背景クリアと、アプリアイコンのホバー状態に応じた描画を行っている。
