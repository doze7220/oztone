...
*   **`void Draw(ID2D1DeviceContext* context, const std::vector<float>& spectrum, D2D1_RECT_F drawRect)`**
    *   8192サイズの高解像度スペクトルデータを、人間の耳の周波数特性に合わせた対数スケールでマッピングし、インデックス間の値を線形補間(Lerp)して滑らかに展開する。
    *   描画領域を1200分割し、両端5%をマージン領域、中央90%をスペクトル表示領域とする。
    *   マージン領域では、音声再生中に微小なランダムノイズ（アナログノイズ）を付与してビリビリ震える表現を加える。
    *   中央領域では、取得した振幅を用いて隣接する頂点のY座標を交互に反転(上・下)させ、心電図の脈打ちを表現する。
    *   領域全体を7等分し、それぞれの色に対して `ID2D1PathGeometry` を用いて一筆書き( `D2D1_FIGURE_BEGIN_HOLLOW` / `D2D1_FIGURE_END_OPEN` )でパスを構築。
    *   `DrawGeometry` を用いて、太い線(グロー)と細い線(芯)の二重描画を行い、ネオンのような発光効果を実現する。
