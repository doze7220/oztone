...
*   **各具象 Widget 実装 (`Widget_AppLogo.h/cpp`, `Widget_TrackInfo.h/cpp` 等)**
    ...
    *   `PlaybackControlsWidget` (`src/Widget_PlaybackControls.h/cpp`), `VolumeControlWidget` (`src/Widget_VolumeControl.h/cpp`): 画面下部ホバー時に白ベクターアイコンでフェードインする再生コントロール（5ボタン構成）および音量コントロールの描画。`PlaybackControlsWidget` では各ボタンごとにホバー状態を追跡し、非対称フェードによる色合成(`HoverIconColor`)を適用している。また、10秒スキップ用の「≪」「≫」ボタンについて、`ConfigManager` から取得した `SkipIconPoints` 等の座標パラメータやテキストパラメータを用いて動的にポリゴン（面）や影付きインジケーターを生成・描画し、鋭利なシルエットとINIからのカスタマイズ性を両立している。`VolumeControlWidget` においては、ボリュームアイコンホバー時にマウスホイール操作のアフォーダンスとして、赤いホイールをアクセントとした精細なベクター描画のアイコン付き吹き出し（ツールチップ）を非対称フェードで上部に表示し、`TooltipWidth` や `TooltipIconSize` 等の Config 設定によりアイコンサイズや吹き出しの寸法を柔軟にカスタマイズ可能なリッチなUIを実現している。
