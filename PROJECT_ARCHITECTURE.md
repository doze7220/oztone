...
*   **UI/ビジュアル仕様方針**:
    ...
    *   下部：シークバー、およびマウスをホバーさせた時のみフェードインして浮かび上がる再生コントロール（前の曲、再生/一時停止、次の曲）。
...
*   **`void Run()`**
    ...
    *   進捗率（progress）と時間表示文字列を計算し、`AudioPlayer` から `GetSpectrumData` で最新の周波数スペクトルデータを取得した上で、それらとウィンドウのホバー状態、再生コントロールのホバー状態、および再生状態（`IsPlaying()`）を `Renderer::Render` に渡して描画を行う。現在はCPU負荷軽減のためループ内に `Sleep(1)` を配置。
...
*   **`LRESULT WindowProc(HWND, UINT, WPARAM, LPARAM)`**
    ...
    *   アプリアイコン上でのクリックイベント（ドラッグ移動）や通常のホバー状態の判定、および再生コントロール領域のクリック判定とメディアコマンド (`APPCOMMAND_MEDIA_*`) の発行を処理する。
...
*   **`void Render(bool isHovered, bool isControlHovered, bool isPlaying, float progress, const std::wstring& timeString, const std::vector<float>& spectrum)`**
    ...
    *   画面下部中央にマウスがホバーされた際、フェードアニメーションとともに `ID2D1PathGeometry` を用いて白のベクターアイコンで再生コントロール（前の曲、再生/一時停止、次の曲）を描画する。
