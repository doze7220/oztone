...
*   **UI/ビジュアル仕様方針**:
    ...
    5. **仮想スクロールと再生確定**: 左下のTrackInfo領域は「高級オーディオのダイヤルのような手触り」を再現する。マウスホバー時のアフォーダンス（ツールチップ）とホイールによるパラパラめくりをサポートし、一定時間入力が停止した段階で対象曲へのジャンプと再生が確定する。
### 2. 禁止事項・ルール (AI向け)
...
アプリケーションの設定（`OZtone.ini`）の読み込み・書き出し・ホットリロードを管理する。全設定の初期値は `ConfigManager_DefaultIni.h` の `DEFAULT_INI_CONTENT` にSSOT化されている。初期ファイル生成時は `WritePrivateProfileStringW` による後続の書き込みとの整合性を保つため、**UTF-16 LE (BOM付き)** 形式でバイナリ出力される。サムネイルのJPEG品質を制御する `ThumbnailJpegQuality` や、プレイリスト上のサムネイルレイアウトを制御する `PlaylistThumbSize`, `PlaylistThumbOffsetX`, `PlaylistThumbOffsetY` などの設定パラメータが追加された。また、プレイリストディレクトリのスナップショット監視によるホットリロード機能を備え、外部からの `.ozl` ファイル（旧 `.lst` フォーマットは完全廃止）の増減や変更を検知して自動的に反映させる。
**物理分割ファイル:**
1. `ConfigManager.cpp`: デフォルト設定文字列とファイルI/Oコア処理
2. `ConfigManager_Window.cpp`: ウィンドウ設定、Z-Order、可視性
3. `ConfigManager_Background.cpp`: 背景アート・クロスフェード設定用
4. `ConfigManager_Playlist.cpp`: プレイリストUI全般
5. `ConfigManager_Playback.cpp`: シークバー、再生・音量コントロール
6. `ConfigManager_LogoMenu.cpp`: アプリアイコン、ロゴ拡張メニュー
7. `ConfigManager_Visualizer.cpp`: ビジュアライザ全般
8. `ConfigManager_System.cpp`: グローバルホットキー、OSDなど

#### `Renderer` クラス (src/Renderer.h)
Direct3D 11 / Direct2D / DirectComposition を用いたハイブリッド描画エンジン。内部ロジックを論理ピクセル（96DPI基準）に統一し、描画の根元で `SetTransform` を用いて一括DPIスケールするアーキテクチャ。ビジュアライザを自前で描画し、それ以外のUI要素は全て `IWidget` 実装クラス群に委譲する。ドラムアニメーション（リングバッファ）のステートマシンは独立した部品として `TrackDrum` クラスにカプセル化され、機能が `Renderer` から切り離されている。これにより、コールバック・イベント駆動によるデータ層とのデカップリングおよび「究極のカプセル化」を実現している。背景描画に関しては、背景の位置や拡大率などの状態変数を1ミリも持たず、また設定の参照処理すらもパージされており、`BackgroundManager` が構築したレイヤー配列（`BackgroundLayer`）を先頭から順次描画し、レイヤーグループのPush/Popコマンドを受動的に解釈して不透明度を一括適用するだけの「完全な受動態（ペインター）」へと究極に純化されている。
**物理分割ファイル:**
...
*   `TrackInfoWidget` (src/Widget_TrackInfo.h/cpp): 左下のアルバムアート・曲名・アーティスト名の描画。データ層への直接アクセス（オンデマンド取得）の責務を剥奪され、コンテキストから渡される独立した3つのスロット (`drumSlots`) の情報を無条件で描画するだけの受動態へ純化された。中間アニメーション時はガラス板のみのフォールバックを描画する。また、テキストレイアウト更新処理 (`UpdateTextLayouts`) についても、既に保護されているスロット内データからのキャッシュ再構築に限定され、毎フレームの外部データ上書きは完全にパージされている。ホバー時はアフォーダンスとして、WidgetCommonの共通ユーティリティを用いた吹き出しツールチップを描画する。
...
タグ解析およびFFT波形事前スキャンをバックグラウンドスレッドで非同期実行し、完了データを `TrackDatabase` へ自律的に書き込むクラス。ファイルI/Oやタグ解析時の未知の例外を捕捉し、スレッドをクラッシュさせずに次の解析を継続する強固なフェイルセーフ機構に加え、一時的なファイルロック競合を回避するための短時間リトライ機構を備える。
...
バックグラウンドスレッドで非同期にサムネイル画像を生成・リサイズし、キューイングされたタスクを順次処理する専用のエンジンクラス。`ThumbnailManager` の配下で稼働する内部コンポーネントであり、外部からの発注（IDとパスのペア）に従って黙々とサムネイルを生成する「純粋な工場（ワーカー）」として機能する。FileManager と連携し、非同期に画像バイナリを抽出・WICデコードしてサムネイルを生成・保存するパイプラインが完全に開通している。
ファイルロック競合時には短時間リトライによって競合回避を試み、それでも画像データ破損等により生成・保存処理を完遂できなかった場合は、速やかにデータベースの発番情報をロールバック（破棄）し、次回のUI更新タイミングで再発注させる自律的なリトライ・自己修復機構を持つ。

#### `BackgroundManager` クラス (src/BackgroundManager.h, cpp)
背景アートの保持、フレーミング状態（拡大率やオフセット）の管理、および描画レイヤーの構築を完全に統括する空間の司令塔クラス。Rendererから背景に関するあらゆる状態と設定依存を引き継いでいる。FileManagerを用いた非同期画像抽出とWICデコードを専用のバックグラウンドスレッドで担当し、UIスレッドをブロックしない。
描画層に対しては、デスクトップ透過を維持するために旧来の下敷きレイヤー（黒塗り）をパージし、D2Dのレイヤーグループ（`PushLayer` / `PopLayer` に対応するマーカー）を用いた構造を動的構築して配列として渡し、Rendererにそれをそのまま描画させる。これにより合成された背景全体に対して `BgOpacity` を一括適用する仕組みへと進化した。クロスフェードは「NEWスワップ式」を採用しており、OLD/NEWそれぞれ独立してフレーミングを保持しつつ、フェード率が1.0fに達した瞬間にNEWの画像と状態をOLDへスワップする安全な機構を持つ。
