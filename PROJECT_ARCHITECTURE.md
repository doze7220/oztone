...
*   **`bool Initialize(HINSTANCE hInstance, int nCmdShow)`**
    ...
    *   起動時、`ConfigManager` からプレイリストのパスを取得し `PlaylistManager::LoadFromFile` でリストを復元。曲があれば自動で最初の曲の再生と次曲の先読みを開始し、無ければUIを空状態(`No Track`)で初期化する。
*   **`void OnFilesDropped(const std::vector<std::wstring>& paths)`**
    ...
    *   追加後にプレイリストをファイル（`DefaultPlaylistPath`）へ自動出力し、もしキューが新規追加によって空から1曲以上になった場合は、即座に最初の曲の再生と次曲の先読みを開始する。
