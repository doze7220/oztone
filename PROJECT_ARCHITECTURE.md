...
5.  **状態管理と画像ロード（フリップサイクル）**: 曲情報の状態管理において、変数のコピー退避（バケツリレー）を廃止し、2つの `DrumSlot` 構造体のインデックスを切り替える「フリップサイクル（ダブルバッファリング）機構」へ刷新。旧来の先読み処理は完全パージされ、画像の非同期ロードはフリップ後のカレントスロットに対してオンデマンドで行うストイックなライフサイクルへ移行。
...
Direct3D 11 / Direct2D / DirectComposition を用いたハイブリッド描画エンジン。内部ロジックを論理ピクセル（96DPI基準）に統一し、描画の根元で `SetTransform` を用いて一括DPIスケールするアーキテクチャ。背景アートとビジュアライザを自前で描画し、それ以外のUI要素は全て `IWidget` 実装クラス群に委譲する。
**物理分割ファイル:**
...
*   `TrackInfoWidget` (src/Widget_TrackInfo.h/cpp): 左下のアルバムアート・曲名・アーティスト名・トラック番号の描画。データ層への直接アクセス（オンデマンド取得）の責務を剥奪され、コンテキストから渡される `DrumSlot` (画像) と `shuffleMetadataList` (テキスト) を描画するだけの受動態へ純化された。
