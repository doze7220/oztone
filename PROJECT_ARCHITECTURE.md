...
*   **各具象 Widget 実装 (`Widgets.h/cpp`)**
    ...
    *   `OsdWidget` (`src/OsdWidget.h/cpp`): 画面中央に配置されるOSD（On-Screen Display）テキスト領域を描画。Z-Order等のホットキー操作やシャッフル切り替え時の状態（Fly Text）を、減衰するアルファ値とともに画面中央に表示し、動的な視覚フィードバックを提供する。
