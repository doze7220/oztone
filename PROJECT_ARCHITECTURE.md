...
アプリケーションの設定（`OZtone.ini`）の読み込み・書き出し・ホットリロードを管理する。全設定の初期値は `ConfigManager_DefaultIni.h` の `DEFAULT_INI_CONTENT` にSSOT化されている。初期ファイル生成時は `WritePrivateProfileStringW` による後続の書き込みとの整合性を保つため、**UTF-16 LE (BOM付き)** 形式でバイナリ出力される。また、プレイリストディレクトリのスナップショット監視によるホットリロード機能を備え、外部からの `.ozl` ファイル（旧 `.lst` フォーマットは完全廃止）の増減や変更を検知して自動的に反映させる。
**物理分割ファイル:**
*   `ConfigManager.cpp`: デフォルト設定文字列とファイルI/Oコア処理
*   `ConfigManager_DefaultIni.h`: 全設定のSSOTとなるデフォルトINI設定値
*   `ConfigManager_Window.cpp`: ウィンドウ設定、Z-Order、可視性
*   `ConfigManager_Background.cpp`: 背景アート・クロスフェード設定用
*   `ConfigManager_Playlist.cpp`: プレイリストUI全般
*   `ConfigManager_Playback.cpp`: シークバー、再生・音量コントロール
*   `ConfigManager_LogoMenu.cpp`: アプリアイコン、ロゴ拡張メニュー
*   `ConfigManager_Visualizer.cpp`: ビジュアライザ全般
*   `ConfigManager_System.cpp`: グローバルホットキー、OSDなど
...
ファイルI/Oおよびメタデータ抽出を単一の窓口として引き受けるクラス。外部に対して TagLib などの存在を隠蔽する Adapter パターンとして機能する。軽量なテキストメタデータの抽出と、重い画像バイナリのオンデマンド抽出に責務を分割している。また、一時的なファイルロック競合を回避するリトライ機構を内部にカプセル化し、I/Oの責務を集約している。
