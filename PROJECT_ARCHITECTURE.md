...
*   **各具象 Widget 実装 (`Widgets.h/cpp`)**
    ...
    *   `OsdWidget` (`src/OsdWidget.h/cpp`): 画面中央に配置されるOSD（On-Screen Display）テキスト領域を描画。音量調整、Z-Order変更、シャッフル切替に加え、再生/一時停止、停止、次/前の曲、プレイリスト切替、アプリ終了など、すべてのホットキーおよびメディアキー操作時の状態（Fly Text）を、減衰するアルファ値とともに画面中央に表示し、動的な視覚フィードバックを提供する。
