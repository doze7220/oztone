...
### 3. プロジェクトフォルダ構造
現在のディレクトリ・ファイル構成は以下の通り（Phase 1 Step 2 完了時点）。

```text
oztone/
 ├── CMakeLists.txt          # CMakeビルド設定
 ├── build/                  # CMakeのビルド出力ディレクトリ (手動で作成・実行)
 ├── src/                    # C++ソースコード
...
*   **`bool GetShowTitleBar() const`**, **`bool GetShowWindowFrame() const`**, **`bool GetShowTaskbar() const`**
    *   設定ファイルから読み込んだ各種ウィンドウの表示フラグを返す。

#### `Renderer` クラス (src/Renderer.h, cpp)
Direct3D 11 と Direct2D を用いたハイブリッド描画エンジン。
*   **`bool Initialize(HWND hwnd)`**
    *   D3D11デバイス、DXGIスワップチェイン、D2D1ファクトリ・デバイス・デバイスコンテキストを生成し、DXGIバックバッファと紐づいたD2D1レンダーターゲットを作成する。
*   **`void Render()`**
    *   毎フレーム呼び出され、描画処理を実行して Present を呼び出す。現在は画面全体を黒でクリアする処理のみ実装。
