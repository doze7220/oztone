...
描画層に対しては「4層レイヤー構造（1.下敷き、2.OLD画像、3.NEW画像、4.ポストエフェクト）」を動的構築して配列として渡し、Rendererにそれをそのまま描画させる。クロスフェードは「NEWスワップ式」を採用しており、OLD/NEWそれぞれ独立してフレーミングを保持しつつ、フェード率が1.0fに達した瞬間にNEWの画像と状態をOLDへスワップする安全な機構を持つ。
