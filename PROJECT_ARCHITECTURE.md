...
*   `Application.cpp`: コンストラクタ、デストラクタなどエントリポイント。デストラクタでは現在アクティブなプレイリストのパスや楽曲状態、データベースなどを確実に保存し、次回起動時の状態復元を担保している。
...
*   **`void SetupCallbacks()`**
    *   ウィンドウに対する各種マウス・キーボード等のコールバック登録を一元管理する。
...
*   **`bool PlayCurrentTrack()`**
    *   メタデータの自己修復、AudioPlayerによる再生開始、次曲の先読み（PrefetchNextTrack）という再生開始の共通3点セットを集約して実行する（フェイルセーフのためboolを返す）。
...
*   **`void ForceRender()`**
    ...
    *   **プレイリスト連携**: `m_isPlaylistListViewMode` フラグを用いて現在のプレイリストUIがどちらのモード（曲一覧・プレイリスト一覧）であるかの状態管理を一元化している。`Window` からツールバーおよびリスト領域でのホイールスクロールや左クリック・ダブルクリックのコールバックを受け取り、現在のモードに基づいてモード遷移、リスト切替、曲・リストの削除、新規作成、ジャンプ再生などの操作を適切に振り分けるアーキテクチャとなっている。プレイリスト操作等の巨大だったコールバックラムダ式は、`OnPlaylistClicked`、`OnPlaylistDoubleClicked`、`OnPlaylistToolbarClicked` などの独立したメンバ関数へ分離・整理されている。リスト操作において、曲一覧モードでは直感的にサクサク再生できるようシングルクリックで決定（フォーカス更新およびジャンプ再生）とし、プレイリスト一覧モードでは誤操作を防ぐためシングルクリックでフォーカスの設定のみを行い、ダブルクリック時に実際のリスト切り替えを行うハイブリッドな仕様としている。リスト領域のクリック判定（インデックス逆算）時には描画の計算式と同じロジックを用い、ツールバーの高さを加味して開始位置をずらしている。またジャンプ再生直後に、ジャンプ前後のインデックス差分から算出した補正値を手動スクロール量へ加算することで、曲の基準点が切り替わった瞬間にリストが視覚的にジャンプしてしまう現象を完全に相殺し、スクロール位置を維持する極上のUXを実現している。
    *   **背景フレーミングの永続化とクランプ管理**: Window 側で検知された右ドラッグやSHIFT＋ホイールによる背景画像のオフセット移動・ズーム情報をコールバックで受け取り、Renderer にクランプ処理を委譲しつつ、**メモリ上の ArtFramingDatabase にリアルタイムに反映する。**ディスクI/O負荷を低減するため、操作のたびにTSVファイルへ保存するのではなく、曲が切り替わる直前（自動進行、手動スキップ、ジャンプ再生）やプレイリストの破棄直前、およびアプリ終了時にのみ一括でセーブを行うよう最適化されている。
*   **`void ClearPlaylist()`**
...
*   **`LRESULT WindowProc(HWND, UINT, WPARAM, LPARAM)`**
    ...
    *   **背景フレーミングの入力フック**: 背景の何もない領域において、右ボタンドラッグ（移動）および「右クリックを押しっぱなし＋マウスホイール」（拡大縮小）を検知し、専用のフレーミング操作としてフックする。また、ディスパッチ処理に追加された `WM_MBUTTONDOWN` と `HandleMButtonDown` により、「右クリックを押しっぱなし＋マウス中ボタンクリック」を検知してフレーミングのリセット操作として処理する。これらの操作は専用のコールバックを介して `Application` へ伝達される。
    ...
    *   システムトレイのアイコンからのメッセージ (`WM_TRAYICON`) を処理し、右クリック時にコンテキストメニューを表示する。メニューの定義（構築順序）は `Window_TrayMenu.cpp` 先頭の `TRAY_MENU_ORDER` 配列で行われており、この配列を編集することで容易に項目の追加・入れ替えが可能となっている。右クリックメニューには、再生/一時停止・前/次の曲といった再生コントロールと音量調整サブメニューが最上部に配置されている。また、`ConfigManager::GetAvailablePlaylists()` を用いて動的に利用可能なプレイリストファイルの一覧を取得し、「プレイリスト (Playlists)」サブメニューとして構築する機能を持つ。Zオーダーの切り替え、位置記憶（終了時に位置とサイズを記憶）のトグルや設定初期化などの高度な設定項目は「詳細設定 (Advanced)」サブメニューに格納されており、メニュー表示時のチェックマーク状態の同期 (`MF_CHECKED` / `MF_UNCHECKED`) と、コマンド実行時の即時状態トグルがシステム設定(`ConfigManager`)と完全に連動するよう実装されている。
...
**※AI-IDEでの開発効率とコンテキスト節約のため、実装対象（UI等）別に以下の7ファイルに物理分割されている。**
...
*   **`bool CheckForUpdates()`**, **`void LoadSettings()`**
    ...
    *   巨大な `LoadSettings` を解体し、各UIの物理分割先ファイル（`ConfigManager_Window.cpp`等）へ委譲してカプセル化している。また、INIオートフィル機構（自己修復機能）のヘルパー関数を導入し、読み込みと追記を1行で完結させるアーキテクチャへ進化した。
    *   **SSOT化とフェイルファスト**: 全ての設定の初期値（デフォルト値）は `ConfigManager_DefaultIni.h` の `DEFAULT_INI_CONTENT`（INI形式のマスターデータ文字列）へSSOT化（Single Source of Truth）されており、各設定ファイル側（`ConfigManager_*.cpp`）でのデフォルト値のハードコードは廃止されている。値の取得時は内部の `GetDefaultIniValue` にてマスター文字列から動的にパースされ、もしマスターデータ側にキーが未定義の場合は `assert` により意図的にクラッシュさせるフェイルファスト設計となっている。
...
*   **`std::wstring GetDefaultPlaylistPath() const`**, **`void SetDefaultPlaylistPath(const std::wstring& path)`**
    *   起動時に読み込むデフォルトのプレイリストファイルのフルパスを取得・設定する。設定が存在しない場合、初期値（`実行ファイルと同階層/oztone_playlist.lst`）を生成し、直ちにINIファイルへ書き出してユーザーが確認・編集できるようにする。`SetDefaultPlaylistPath` は新規プレイリスト作成時や切り替え時の他、アプリケーション終了時にも明示的に呼ばれ、現在のパスをINIファイルへ確実に永続化更新することで次回起動時の状態復元を担保する。
...
**※巨大化していた `Renderer.cpp` は、AI-IDEのコンテキスト節約および可読性向上のため、クラス設計（.h）はそのままに、実装（.cpp）が以下のファイル群へ物理分割されている。**
...
*   `Renderer.cpp` 本体: コンストラクタ・デストラクタやセッター等の最小限の窓口機能のみが残されている。
...
#### `Widget` コンポーネント (src/Widget.h, src/WidgetContext.h, src/WidgetCommon.h/cpp, src/Widget_*.h/cpp)
...
*   **`WidgetCommon` ユーティリティ (`src/WidgetCommon.h / .cpp`)**
    *   特定の Widget クラスに依存しない共通のステートレスなユーティリティ処理を集約・隔離する名前空間。
    *   WIC画像読み込みヘルパー、DWriteテキストフォーマットのトリミング適用、Hex色文字列変換、および複数Widgetで頻出する「影用ブラシ設定→影描画→本体描画」といった影付きテキスト描画のボイラープレートなどを共通化して提供し、各Widgetのコード量削減と可読性向上を担う。
*   **各具象 Widget 実装 (`Widget_AppLogo.h/cpp`, `Widget_TrackInfo.h/cpp` 等)**
    *   `AppLogoWidget` (`src/Widget_AppLogo.h/cpp`): アプリアイコンの描画。メニュー展開中（`isLogoMenuHovered`がtrueの時）は、大元のアプリアイコンもホバー状態画像（`app_logo_hover.png`）を維持する。
    *   `LogoMenuWidget` (`src/Widget_LogoMenu.h/cpp`): アプリアイコンホバー時に右側へアニメーション進行度に応じたイージングでスライド展開する「ロゴ拡張メニュー」の描画。配列ベースで管理されたメニューアイコンの展開、ホバー時における背面グロー画像（`app_logo_back.png`）の合成、トグルOFF時（またはビジュアライザ無効時）の視覚フィードバック（半透明化）、およびビジュアライザ・背景表示アイコン等の右下のインジケーター等の縁取り描画を担当する。また、各メニューアイコンのホバー時に非対称フェードで個別に色を合成・補間(`HoverIconColor`等を利用)し、状態（プレイリスト左右配置等）やホバーアイコンに連動した動的なテキスト説明表示（シャドウ付き）を行う。
    *   `TrackInfoWidget` (`src/Widget_TrackInfo.h/cpp`): 左下のアルバムアート、曲名・アーティスト名、およびトラック番号（「XXX/XXX」表記、アライメント変更対応）の描画。正規のアルバムアートがない場合はプレースホルダー領域として黒い板（`FallbackArtOpacity`）を描画するフォールバック処理を行う。テキストトリミング機能を含む。また、IDWriteTextLayoutによるテキストのキャッシュ化を行い、描画ループ内での動的リソース生成を排除している。
    *   `NextTrackWidget` (`src/Widget_NextTrack.h/cpp`): 右下の「次の曲」の極小アルバムアートと曲情報描画。設定により非表示時はスキップ。
    *   `SeekBarWidget` (`src/Widget_SeekBar.h/cpp`): 進行度に応じたシークバーと現在時間の描画。ホバー領域に入ると輝度が最大50%減衰する機能を持つ。
    *   `PlaybackControlsWidget` (`src/Widget_PlaybackControls.h/cpp`), `VolumeControlWidget` (`src/Widget_VolumeControl.h/cpp`): 画面下部ホバー時に白ベクターアイコンでフェードインする再生コントロール（5ボタン構成）および音量コントロールの描画。`PlaybackControlsWidget` では各ボタンごとにホバー状態を追跡し、非対称フェードによる色合成(`HoverIconColor`)を適用している。また、10秒スキップ用の「≪」「≫」ボタンについて、`ConfigManager` から取得した `SkipIconPoints` 等の座標パラメータやテキストパラメータを用いて動的にポリゴン（面）や影付きインジケーターを生成・描画し、鋭利なシルエットとINIからのカスタマイズ性を両立している。`VolumeControlWidget` においては、スピーカーアイコンに対しても再生コントロール等と同様にホバー時の非対称カラーフェードアニメーションによる色合成（`HoverIconColor` を用いた Lerp 補間）が適用され、UI全体の手触り（アフォーダンス）が統一された。加えて、ボリュームアイコンホバー時にマウスホイール操作のアフォーダンスとして、赤いホイールをアクセントとした精細なベクター描画のアイコン付き吹き出し（ツールチップ）を非対称フェードで上部に表示し、`TooltipWidth` や `TooltipIconSize` 等の Config 設定によりアイコンサイズや吹き出しの寸法を柔軟にカスタマイズ可能なリッチなUIを実現している。
    *   `GlobalHotkeysWidget` (`src/Widget_GlobalHotkeys.h/cpp`): 設定で有効時、右上にグローバルホットキー設定値を一覧で描画するチートシート機能。`ConfigManager` から取得した `[Layout_GlobalHotkeys]` のパラメータ (`GlowOpacity`, `KeyColumnOffset`, `ActionColumnOffset`) を用いて動的に表示位置と透明度を計算し、テキストをキー名と機能名に分割して2カラムの左寄せ表組みレイアウトとして描画する。`PlaylistWidget` の奥側に高いZ-Orderで配置される。
    *   `PlaylistWidget` (`src/Widget_Playlist.h/cpp`): INI設定(`PlaylistPosition`)に応じ、画面左端または右端のホバー時にスライドインするシャッフル再生リストと「引き出しグリップ」の描画。グリップの矢印の向きも左右配置に応じて動的に反転する。`Application` から渡されたモード情報（`isPlaylistListViewMode`）に基づき、曲一覧またはプレイリスト一覧を切り替えて描画し、上部に固定ツールバー（上段にアイコン、下段に日本語説明テキスト）を描画する純粋な描画責務を持つ。ツールバーには「表示固定（ピン留め）」機能のトグルボタンも備えられ、現在の状態（自動格納／画面固定）に応じたサブアイコン（🔓/🔒）およびツールチップを描画する。曲一覧モードの非ホバー時には、設定から取得した現在のプレイリストパスを直接パース（拡張子を除外）してツールバーにプレイリスト名を常時表示する。自身でスライドインアニメーションやスクロール制御、限界値クランプ、各テキストの描画を担当し、対象曲のメタデータが解析済み（`isLoaded == true`）の場合は取得した曲名・アーティスト・再生時間を使用してリッチに描画、未解析の場合はファイル名から推測したタイトルでのフォールバック描画を行う。また、描画ループ内でアイテムのホバー状態（`HoverItemColor`）および再生中状態（`PlayingItemColor`）に基づき、対象アイテムのみのアルファ値を非対称フェードで計算・合成することで、数千件のプレイリストでも高パフォーマンスに色遷移を実現している。プレイリスト一覧モード時においても、`Application`側で非同期・軽量にパースされ `WidgetContext` 経由でキャッシュとして供給される `PlaylistSummary` （表示名、曲数、総時間）を利用し、上段にプレイリスト名、下段に曲数と総再生時間を配置するリッチな2行スタイルで描画を行う。また、ピン留めが有効な状態では、格納を想起させる引き出しグリップの矢印描画をスキップしアフォーダンスを調整している。巨大化していた描画ループ(`Draw()`)が完全に解体され、各専用メソッドへ細分化されたのち、**AI-IDEのコンテキスト節約と可読性向上のため、クラス設計（.h）はそのままに、実装（.cpp）が責務ごとに以下の6つのファイルに物理分割されている。**
        ...
        *   `Widget_Playlist_DrawItems.cpp` : プレイリストの曲一覧およびリスト一覧のアイテム描画
        *   `Widget_Playlist_Draw.cpp` : 大元の描画ループと背景などのベース描画
        *   `Widget_Playlist.cpp` (本体) : コンストラクタや最小限の窓口機能のみ
    *   `ResizeGripWidget` (`src/Widget_ResizeGrip.h/cpp`): リサイズモード有効時に右下隅に半透明のグリップを描画。
    *   `OsdWidget` (`src/Widget_Osd.h/cpp`): 画面中央に配置されるOSD（On-Screen Display）テキスト領域を描画。音量調整（操作後の現在ボリュームの絶対値表示）、Z-Order変更、シャッフル切替に加え、再生/一時停止、停止、次/前の曲、プレイリスト切替、アプリ終了など、すべてのホットキーおよびメディアキー操作時の状態（Fly Text）を、減衰するアルファ値とともに画面中央に表示し、動的な視覚フィードバックを提供する。
