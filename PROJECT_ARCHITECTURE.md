...
Direct3D 11 と Direct2D を用いたハイブリッド描画エンジン。画像デコード用に WIC、テキスト描画用に DirectWrite を内包する。
*   **`bool Initialize(HWND hwnd, const ConfigManager& config)`**
    *   D3D11デバイス、DXGIスワップチェイン、D2D1ファクトリ等を作成。WICファクトリを初期化し、初期アセット（アプリアイコンやプレースホルダーアートなど）を読み込んで D2D1Bitmap を準備する。さらに、DirectWrite ファクトリを初期化し、テキストフォーマットやブラシを準備する。
...
*   **`void Render(bool isHovered)`**
    *   毎フレーム呼び出され、描画処理を実行。現在は背景クリア後、背景アルバムアート(Cover表示)、ホバー状態に応じたアプリアイコン、左下のアルバムアート、およびダミーの曲情報テキストの順に描画を行っている。
