...
*   **`void ParseThreadFunc()` (バックグラウンド解析スレッド)**
    *   起動時やファイルドロップ時に追加された未解析トラックを専用キューから順次取り出す。ここでタグ解析（`TagManager`）とFFTスキャン（`AudioPlayer::ScanAudioData`）は完全に独立したステップとして実行される。
    *   **競合対策**: 即時再生時のファイルロック競合（TagLibの共有アクセス違反）によってタグ読み込みが失敗した場合でも、裏スレッドは確実にFFTスキャンのみを完遂して波形データを供給する堅牢なアーキテクチャとなっている。
    *   未解析トラックの処理が完了しキューが空になったタイミングで、ファイルへの一括永続化保存（TSV出力）を行う。これにより大量のファイル追加時にもUIスレッドをブロックしない。
...
*   **`void Run()`**
    ...
    *   `AudioPlayer` の再生終了（`IsAtEnd`）を検知した場合、かつ `m_isPrefetchReady` が `true` になっていることを確認した上で、プレイリストを進め、先読みデータを `Renderer` の「現在の曲」情報として即時反映し、次の曲を再生しつつ更に次の曲を先読みする。これにより、重いWICデコード等による曲間移行時のUIブロックを防いでいる。また、再生終了時には再生中に収集した自己学習データ（ピーク振幅・最大周波数）を評価し、既存のメタデータが未解析または学習値が既存値を上回る場合にはメタデータを更新し、自動保存を行う（事前スキャンがOFFでも再生するだけで学習が行われる）。
...
*   **`VisualizerHaloDust` (円形パーティクル表現)**
    ...
    *   256バンドの周波数帯を間引きながら放射状のバーを描画し、ビート検知時にレーザー(`LaserRay`)や3D回転する破片パーティクル(`Particle`)を全方位へ放出する。パーティクルはCPU上でロドリゲスの回転公式を用いて計算される。
...
内部のリストデータは `TrackMetadata` 構造体（ファイルパス、曲名、アーティスト名、再生時間、**タグ解析用フラグ(`isMetaLoaded`)と波形解析用フラグ(`isFFTLoaded`)の独立した管理**、**アートのフレーミング情報(オフセット・スケール)**、**ビジュアライザ用の解析済みメタデータ(ピーク振幅・最大周波数)** を保持）で管理され、`std::mutex` によりマルチスレッドからの安全な排他制御アクセスが保証されている。これらの情報はTSVとして永続化され、曲ごとにカスタマイズされたアートの位置や拡大率、波形データを維持する。
