...
`TagLib` をラップし、音声ファイルからID3タグ（曲名・アーティスト名等）のテキスト情報のみを抽出する純粋なパーサーへと純化されたクラス（画像抽出の責務は完全にパージ済み）。今後は `FileManager` へ解体・吸収される予定。

#### `FileManager` クラス (src/FileManager.h)
ファイルI/Oおよびメタデータ抽出を単一の窓口として引き受ける門番クラス。外部に対してTagLib等の外部ライブラリの存在を完全に隠蔽し、Adapterパターンとして機能する。大量の楽曲ロード時のメモリ圧迫を防ぐため、軽量なテキストメタデータの抽出（`ExtractTextMetadata`）と、重い画像バイナリのオンデマンド抽出（`ExtractAlbumArtBinary`）に責務を分割して定義されている。現在インターフェースのみ定義済み。
